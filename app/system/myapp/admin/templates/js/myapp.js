			...
			success: function (result) {
				if (!result.data) {
					$list.html(`<div class="tips">
						...
						<div class="mt-2"><a href="#/myapp/login" onClick="setCookie('app_href_source','myapp/?head_tab_active=0')" class="btn btn-primary py-1">${METLANG.loginconfirm}</a></div>
					</div>`);
					return;
				}
				...
				data.map(item => {
					const card = `<div class="col col-6 col-xl-4 col-xxl-3 mb-3 px-2" >
							<div class="media bg-white h-100 flex-column transition500 rounded-20 p-4 ${item.url ? 'install' : ''}" data-no="${item.no}" data-m_name="${item.m_name}" data-new_ver="${item.new_ver ? item.new_ver : ''}">
								<div class="body media-body w-100">
									<a href="${item.url ? item.url : 'javascript:;'}" ${parseInt(item.target) ? 'target="_blank"' : ''} class="link w-100 d-flex"  title="${METLANG.fliptext1}" data-newapp="${item.newapp ? item.newapp : ''}">
										<img class="mr-3 rounded-10" width="70" height="70" src="${item.icon}">
										<div class="media-body cover">
											<h5 class="h6 mt-1 d-flex">
												<span class='mr-2 media-body'>${item.appname}</span>${item.ver?`<span class='text-grey font-size-14 version'>v${item.ver}</span>`:''}
											</h5>
								...
								<ul class="actions w-100 mt-3 rounded-10 d-flex ${!item.install&&!item.enabled?'bg-grey-100':''}">
								${
									!item.install
									? `<li class="${item.enabled?'btn-install':'text-help'}">${item.enabled?`<a href="javascript:;" class='d-block'>`:''}<span class="${item.enabled?`ml-2`:'font-size-12'}">${item.enabled?METLANG.appinstall:item.btn_text}</span>${item.enabled?'<i class="fa fa-cloud-download ml-1"></i></a>':''}</li>`
									: `
									${item.new_ver ? `<li><a href="javascript:;" class='update'><span class="text-dark">${METLANG.appupgrade}</span><i class="fa fa-arrow-up ml-1"></i></a></li>` : ``}
									${item.system ? `<li class="text-black-50">${METLANG.system_plugin_uninstall}</li>` : `<li class="uninstall"><a href="javascript:;" class="text-black-50 d-block"><span class="ml-2">${METLANG.dlapptips6}</span><i class="fa fa-trash ml-1"></i></a></li>`}`
								}
								</ul>
							</div>
						</div>`;
	...
	function deleteApp() {
		$list.on('click','.uninstall',function () {
			...
			M.load('alertify', function () {
				alertify.addHead();
				alertify.confirm(M.synchronous=='cn'?'确定要卸载该插件吗？卸载后插件文件和数据会一并删除且无法恢复，请谨慎操作！':'Are you sure you want to uninstall this plugin? After uninstallation, the plugin files and data will be deleted together and cannot be recovered. Please handle with caution!', function (ev) {
					M.ajax({
						url: that.own_name + 'c=index&a=doAction',
						data: {
				...
				if (result.status) {
					...
					var user =  $('.met-myapp-right'),
						userHtml = `<a href="#/myapp/login" onClick="setCookie('app_href_source','myapp/?head_tab_active=0')" class="mr-2">
							<button type="button" class="btn btn-default py-1">${METLANG.loginconfirm}</button>
						</a>
