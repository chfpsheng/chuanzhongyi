	...
	function renderList(data) {
		...
		data.map(item => {
			var card = `<div class="col col-6 col-xl-4 col-xxl-3 mb-3 px-2" >
					<div class="media bg-white h-100  flex-column transition500 rounded-20 p-4" data-no="${item.product_code}" data-m_name="${item.m_name}">
						<div class="body media-body w-100">
							<a href="${item.show_url}" class="link w-100 d-flex"  title="${METLANG.fliptext1}" target="_blank">
								<img class="mr-3 rounded-10" width="70" height="70" src="${item.product_image}">
						...
						<ul class="actions w-100 mt-3 rounded-10 d-flex ${item.enabled?'':'bg-grey'}">
							<li class="${item.enabled?'btn-install':'text-help'}">${item.enabled?`<a href="javascript:;" class='d-block'>`:''}<span class="${item.enabled?`ml-2`:'font-size-12'}">${item.btn_text}</span>${item.enabled?'<i class="fa fa-cloud-download ml-1"></i></a>':''}</li>
						</ul>
					</div>
				</div>`;
			html += card;
									...
									<div class="modal-dialog modal-dialog-centered">
										<div class="modal-content">
											...
											<div class="modal-footer justify-content-center">
												<button type="button" data-dismiss="alert" id="${btn_id}" class="btn btn-default py-1 mr-5">${METLANG.cancel}</button>
												<a href="${url}" target="_blank" onClick="$('#${btn_id}').click();" class="btn btn-primary py-1">${METLANG.sys_purchase}</a>
											</div>
										</div>
									</div>
								</div>
				...
				if (result.status) {
					var user = $(".met-myapp-right"),
						userHtml = `<div class="d-flex user">
							...
							<button type="button" class="btn btn-default py-1 btn-logout">${METLANG.indexloginout}</button>
						</div>`;
					...
					var user = $(".met-myapp-right"),
						userHtml = `<a href="#/myapp/login" onClick="setCookie('app_href_source','myapp/?head_tab_active=2')" class="mr-2">
