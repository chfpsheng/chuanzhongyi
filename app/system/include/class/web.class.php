...
class web extends common
{
    ...
    public function __construct()
    {
        ...
        // 可视化状态下iframe地址未带pageset=1的页面跳转
        if (strstr($_SERVER['HTTP_REFERER'], $_M['url']['web_site']) && strstr($_SERVER['HTTP_REFERER'], 'pageset=1') && !strstr($_SERVER['QUERY_STRING'], 'pageset=1') && !(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')) {
            $http = $this->checkHttps();
            $location = $http . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'].(strstr($_SERVER['QUERY_STRING'], '?')?'?':'&').'pageset=1';
            header('location:' . $location);
            die;
        }
    ...
    protected function sys_input()
    {
        ...
        $this->input['page'] = $_M['form']['page'] ? intval($_M['form']['page']) : 1;
        $this->input['id'] = $_M['form']['id'] ? intval($_M['form']['id']) : 0;
        $this->input['lang'] = $_M['form']['lang'] ? $_M['form']['lang'] : $_M['config']['met_index_type'];
        $this->input['synchronous'] = $_M['langlist']['web'][$this->input['lang']]['synchronous'];
        if ($this->input['classnow']) {
            $column = load::mod_class('column/column_database', 'new')->get_column_by_id($this->input['classnow']);
        }
     ...
     * @return string
     */
    protected function getCustomTemp($file = '', $data = array())
    {
        if (!$data['style_type']) return $file;
        //栏目样式
        $column_style = load::mod_class('column/column_style', 'new')->getStyleTemp($data['module'], $data['style_type']);
        $custom_temp = $column_style ?: $file;
        return $custom_temp;
    }
     ...
     * @return mixed
     */
    protected function webUrlReplace($data = array())
    {
        global $_M;
    ...
    public function view($file = '', $data = array())
    {
        ...
        if ($_M['config']['met_skin_user']) {
            if (!defined('IN_ADMIN')) { //相对路径
                $file = self::getCustomTemp($file, $data);
                self::webUrlReplace($data);
            }
    ...
    protected function check($groupid = 0)
    {
        ...
        if ($power < 0) {
            ...
            if ($power == -2) {//跳登录
                $http = $this->checkHttps();
                $gourl = urlencode($http . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
                okinfo($_M['url']['site'] . 'member/index.php?gourl=' . $gourl . $lang, $_M['word']['systips1']);
            }
            if ($power == -1) {//跳首页
                okinfo(-1, $_M['word']['systips2']);
     ...
     * @return mixed
     */
    protected function input_class($id = null)
    {
        global $_M;
        ...
        if (is_numeric($id)) {
            ...
            $column = $column_lable->get_column_folder($folder);
            if ($column) {
                $cid = $column['id'];
            } else {
                $mod = $_M['class']['handle']->file_to_mod($folder);
                if(!$mod){
                    abort(404);
                }
                if (in_array($mod, array(2, 3, 4, 5, 14, 15, 16))) {
        ...
        //获取三级栏目信息
        $c123_releclass = $column_lable->get_class123_reclass($cid);
        ...
        $c123 = $column_lable->get_class123_no_reclass($cid);
        ...
        //额外栏目信息
        if (in_array($column['module'], array(2, 3, 4, 5, 6, 7, 11, 14, 15, 16))) {
            self::classExt($column);
        }

        //栏目
        if ($column) {
            $this->add_input('module', $column['module']);
            $this->add_input('bigclass', $column['bigclass']);
            $this->add_input('index_num', $column['index_num']);
            $this->add_input('class_name', $column['name']);
            $this->add_input('folder_name', $column['foldername']);
        }
        return $column;
    }
    ...
    protected function folderCheck()
    {
        $REQUEST_URIs = explode('/', REQUEST_URI);
        $folder = $REQUEST_URIs[count($REQUEST_URIs) - 2];
        if ($this->input['folder_name'] == $folder) {
            return true;
        }
        return false;
    }
    ...
    public function showpage($module)
    {
        ...
        if(!$this->folderCheck($data)) abort();
    ...
    public function listpage($module)
    {
        ...
        if ($_M['form']['pseudo_jump']) {//详情页
            if ($_M['form']['list'] != 1) {
                ...
                if (!is_numeric($_M['form']['metid'])) {//根据静态页名称获取栏目信息
                    $metid = daddslashes($_M['form']['metid']);
                    $column = $_M['class']['column_label']->get_column_by_filename($metid);
                    if ($column) {
        ...
        if (!is_numeric($classnow)) {//根据栏目名称获取内容列表
            $metid = daddslashes($_M['form']['metid']);
            $custom = $_M['class']['column_label']->get_column_folder($metid);
            $classnow = $custom['0']['id'];
        }
        $data = $this->input_class($classnow);
        if (!$data) abort();
    ...
    {
        global $_M;
        if (isset($_M['form']['search'])) {// 标签聚合页面TDK
            $search_title = $_M['form']['searchword'] ? $_M['form']['searchword'] : $_M['form']['content'];
            if ($_M['form']['search'] == 'tag' || @$_GET['search'] == 'tag') {
                $tags_lable = $tag = load::sys_class('label', 'new')->get('tags');
                $tag = $tags_lable->getTagInfo($_M['form']['content'], $this->input);
                if (!$tag) {
                    $tag = $tags_lable->getTagInfo($_M['form']['searchword'], $this->input);
                }
                if ($tag) {
            ...
            $search_title = htmlentities($search_title);
            if ($this->input['module'] != 11 && $search_title) {
                $title = $search_title;
            }
            if ($tag['title'] && $search_title) {
                $title = $search_title;
            }
        }
        ...
        switch ($_M['config']['met_title_type']) {
            case '0':
                ...
                break;
            case '1':
                $title_str = $title . '-' . $_M['config']['met_keywords'];
                break;
            case '2':
                $title_str = $title . '-' . $_M['config']['met_webname'];
                break;
            case '3':
                $title_str = $title . '-' . $_M['config']['met_keywords'] . '-' . $_M['config']['met_webname'];
                break;
            case '4':
                if (defined('LIST_PAGE')) {
                    $title_str = $title . '-' . $_M['config']['met_webname'];
                }elseif(defined('SHOW_PAGE')){
                    $title_str = $title . '-' . $this->input['class_name'] . '-' . $_M['config']['met_webname'];
                }else{
                    $title_str = $title . '-' . $_M['config']['met_webname'];
                }
        }
        $this->add_input('page_title', $title_str);

        $keywords_str = $keywords ?: $_M['config']['met_keywords'];
        $this->add_input('page_keywords', $keywords_str);

        $description_str = $description ?: $_M['config']['met_description'];
        $this->add_input('page_description', $description_str);
    }
     ...
     * @param string $url
     */
    protected function seo_canonical($url = '')
    {
        global $_M;
        if ($_M['form']['page']) return;
        if ($_M['form']['search']) return;
        if (!$_M['config']['met_seo_canonical']) return;

//        $search = $_M['form']['search'] ? strtolower($_M['form']['search']) : null;
//        if ($search) {
//            if ($search != 'tag') return;
//            if (!$_M['config']['met_pseudo']) return; //开启伪静态
//            $rel_url = trim($_M['url']['web_site'], '/') . $_SERVER['REQUEST_URI'];
//        } else {
//            $rel_url = $_M['url']['web_site'] . str_replace('../', '', $url);
//        }
        if($_M['config']['met_pseudo']){
            $url=explode('?',$_SERVER['REQUEST_URI']);
            $rel_url = $_SERVER['HTTP_HOST'].$url[0];
        }else{
            $rel_url = $_M['url']['web_site'] . str_replace('../', '', $url);
        }
        $canonical = "<link rel=\"canonical\" href=\"{$rel_url}\">";

        $this->add_input('canonical', $canonical);
        $_M["html_plugin"]["head_script"] .= $canonical;
    }

    /**
     * title 页码
     */
    protected function page_num_title()
    {
        global $_M;
        $page_num = isset($_M['form']['page']) ? intval($_M['form']['page']) : null;
        if (!$page_num) {
            return;
        }
        $page_num_title = isset($_M['word']['page_num_title']) ? $_M['word']['page_num_title'] : ($_M['lang'] == 'cn' ? "第{page_num}页" : "page{page_num}");
        $page_str = str_replace('{page_num}',$page_num,$page_num_title);
        $this->seo_title($this->input['page_title'] . "-{$page_str}");
    }
     ...
     */
    protected function video_replace($preg, $content)
    {
    ...
    public function __destruct()
    {
        ...
        if ($_M['form']['html_filename'] && $_M['form']['metinfonow'] == $_M['config']['met_member_force']) {
            ...
            // 严格校验文件后缀:限制 html_filename 只能为.html或 .htm 后缀
            ...
            $target_path = realpath(PATH_WEB) . DIRECTORY_SEPARATOR . $filename;
            $target_dir = dirname($target_path);
            if (!is_dir($target_dir)) {
                @mkdir($target_dir, 0755, true);
            }
            $real_target = realpath($target_dir);
            $real_webroot = realpath(PATH_WEB);
            if ($real_target === false || strpos($real_target, $real_webroot) !== 0) {
                jsoncallback(array('suc' => 0));
            }
