...
class DB
{
    ...
    public static function query($sql)
    {
        ...
        if (strtoupper(substr($sql, 0, 6)) == 'INSERT') {
            ...
            // 统一处理表名和字段名的反引号（兼容达梦双引号）
    $sql = preg_replace_callback('/`([^`]+)`/', function ($matches) {
        return '"' . str_replace('"', '""', $matches[1]) . '"';
    }, $sql);

    // 1. 处理 INSERT INTO ... SET 语法
    if (preg_match('/INSERT\s+INTO\s+"?(\w+)"?\s+SET\s+(.*?)(;)?$/i', $sql, $matches)) {
        $table = $matches[1];
        $setClause = $matches[2];
        
        // 分割键值对（兼容含逗号的字符串）
        $pairs = preg_split('/,\s*(?=(?:[^\']*\'[^\']*\')*[^\']*$)/', $setClause);
        ...
        foreach ($pairs as $pair) {
            ...
            $col = trim(trim($col), '"'); // 去除字段名的引号
            $val = trim($val);
            
            // 处理自增列（如 id=NULL）
            if (strtoupper($val) === 'NULL' && $col === 'id') {
                continue; // 达梦自增列无需显式赋值
            }
    ...
    // 2. 替换MySQL特定函数（如 NOW() -> SYSDATE）
    $functionMap = [
        '/\bNOW$$/i' => 'SYSDATE',
        '/\bCURRENT_TIMESTAMP\b/i' => 'SYSTIMESTAMP',
        '/\bAUTO_INCREMENT\b/i' => 'GENERATED ALWAYS AS IDENTITY' // 自增列语法修正
    ];
    foreach ($functionMap as $pattern => $replacement) {
        $sql = preg_replace($pattern, $replacement, $sql);
    }

    // 3. 处理字符串转义（单引号转双单引号）
    $sql = preg_replace_callback("/VALUES\s*$(.*?)$/i", function ($matches) {
        $values = preg_replace("/'(.*?[^\\\\])'/", "'" . addslashes('$1') . "'", $matches[1]);
        return "VALUES ($values)";
    }, $sql);
            // 5. 显式事务控制
            dm_exec(self::$link, "SET AUTOCOMMIT OFF");
            dm_exec(self::$link, "BEGIN TRANSACTION");
            
            // 6. 执行并处理错误
            $result = dm_exec(self::$link, $sql);
            if (!$result) {
                dm_rollback(self::$link);
                error("Insert failed: " . dm_error(self::$link));
            } else {
                dm_commit(self::$link);
            }
...
//            self::errno();
    ...
    public static function escapeDmsql($sql)
    {
        $sql = str_replace("\\'", "''", $sql);
        $sql = str_replace('\\', '', $sql);
