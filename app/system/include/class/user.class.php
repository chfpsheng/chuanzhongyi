...
class user
{
    ...
    public function insert_uesr($username = '', $password = '', $email = '', $tel = '', $valid = '', $groupid = '', $source = '')
    {
        if (!$this->check_username($username)) {
            $this->errorno = "username invalidate";
            return false;
        }

        if (!$this->check_password($password)) {
            $this->errorno = "password invalidate";
            return false;
        }
        ...
        if ($isplugin) {
            # 系统账号注册接口
            $registerres = load::plugin('doregister', 1, array($username, $password, $email));
        }
        # $registerres = NULL    说明插件不存在
        # $registerres = false   说明注册失败
        # $registerres = ture    说明注册成功
        if ($registerres === false) {
            $this->errorno = "uesr create failed";
            return false;
        }
     ...
     * @return bool|int
     */
    public function insert_uesr_sql($username = '', $password = '', $email = '', $tel = '', $valid = '', $groupid = '', $source = '', $register_time = '', $register_ip = '', $login_time = '')
    {
        global $_M;
        ...
        $register_time = $register_time ?: $time;
        $register_ip = $register_ip ?: get_userip();
        ...
        if (DB::query($query)) {
            $id = DB::insert_id();
            load::plugin('doregistert', 0, array($id, $username, $password, $email));
            return $id;
        } else {
            load::plugin('doregisterf', 0, array($username, $password, $email));
            $this->errorno = "uesr create failed";
            return false;
        }
    }
     ...
     * @return bool
     */
    public function setUserPass($userid, $password = '')
    {
        global $_M;
        if (!$this->check_password($password)) {
            return false;
    ...
    /**
     * 登录 返回会员信息 $type 等于md5时，是进行加密后的验证
     * @param string $username
     * @param string $password
     * @param string $type
     * @return array|bool
    ...
    {
        global $_M;
        if (!$this->check_str($username)) return false;
        ...
        switch ($type) {
            case 'md5':
                if (md5($user['password']) !== $password) return false;
                break;
            case 'pass':
            default:
     ...
     * @return array
     */
    private function userAttr($user = array())
    {
        global $_M;
        //完善会员信息的头像地址
        $head_img = $_M['url']['public_images'] . 'user.jpg';
        if ($user['head'] && substr($user['head'], 0, 3) == '../') {
            $head_img = $_M['url']['site'] . str_replace('../', '', $user['head']);
        }
        $user['head'] = $head_img;
        return $user;
    }
    ...
    {
        return $this->setLoginRecord($user);
    }
     ...
     * @param array $user
     */
    public function setLoginRecord($user = array())
    {
        global $_M;
        $login_time = time();
    ...
    public function get_user_by_username($username)
    {
        ...
        if (!$user) {
            ...
            $user = $this->get_user_by_emailid($username);
            if (!$user) {
    ...
    public function get_admin_by_openid($openid = '')
    {
        global $_M;
        $tel = addslashes(stripslashes($openid));
        $query = "SELECT * FROM {$_M['table']['admin_table']} WHERE openid='{$openid}'";
    ...
    public function check_power($groupid = 0)
    {
        ...
        // 当 groupid 大于 0 时进行权限检查
        if ($groupid > 0) {
            // 如果管理员已登录，直接返回 1
            if (!empty($_M['admin']['username'])) {
                return 1;
            }

            // 如果未定义 SHOW_PAGE 且访问类型为 1，返回 0
            if (!defined('SHOW_PAGE') && $_M['config']['access_type'] === 1) {
                return 0;
            }
            ...
            // 如果未获取到会员组信息或者用户访问权限小于会员组访问权限，返回 -1
            if (empty($group) || $user['access'] < $group['access']) {
                return -1;
            }
        }
