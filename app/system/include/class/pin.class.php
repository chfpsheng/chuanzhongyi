...
class pin
{
     ...
     * @return bool
     */
    public function check_pin($pin = '', $key = '')
    {
        global $_M;
        $authpin = $this->getpin($key);
        if ($authpin && strtoupper($authpin) == strtoupper($pin)) {
            $this->setpin($key, $this->make_rand(4));
            return true;
        } else {
            return false;
        }
     ...
     * @param string $key
     */
    public function create_pin($key = '')
    {
        $this->random_str = $key;
        $this->getAuthImage();
    }
    ...
    function getAuthImage()
    {
        ...
        $font = PATH_PUBLIC . 'third-party/fonts/Roboto/Roboto-Regular.ttf';
     ...
     * @return string
     */
    function make_rand($length = 32, $type = null)
    {
        if ($type) return random($length, $type);
