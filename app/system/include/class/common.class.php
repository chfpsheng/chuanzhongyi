...
class common
{
    ...
    protected function load_mysql()
    {
        ...
        if ($_M['config']['db_type'] == 'sqlite' && class_exists('SQLite3')) {
            ...
            DB::dbconn($con_db_host, $con_db_id, $con_db_pass, $con_db_name, $con_db_port);
        } elseif ($_M['config']['db_type'] == 'pgsql') {
            load::sys_class('pgsql');
            DB::dbconn($con_db_host, $con_db_id, $con_db_pass, $con_db_name, $con_db_port);
        }
    ...
    protected function load_form()
    {
        ...
        if (is_numeric($_M['form']['lang'])) { //伪静态兼容
            ...
            if ($_M['form']['lang'] && !is_string($_M['form']['lang'])){
                abort();
            }
        }
        ...
        if (!preg_match('/^[0-9A-Za-z]+$/', $_M['form']['lang']) && $_M['form']['lang']) {
            $_M['form']['lang'] = $_M['config']['met_index_type'];
        }
    }
        ...
        if (M_MODULE == 'admin' || (M_MODULE == 'include' && M_CLASS == 'loadtemp')) {
            if ($_M['form']['lang']) {
                if ($_M['form']['lang'] != $_M['form']['admin_lang']) {
                    setcookie('admin_lang', $_M['form']['lang'], 0, '/');
                }
    ...
    protected function load_url_other()
    {
        ...
        $_M['url']['public_third'] = $_M['url']['public'] . 'third-party/';
    ...
    protected function load_group()
    {
        global $_M;
        $_M['max_access'] =  cache::get('data/max_access_' . $_M['lang']);
        if (!$_M['max_access']) {
            $query = "SELECT max(`access`) as `access`  FROM {$_M['table']['user_group']} WHERE lang='{$_M['lang']}'";
            $result = DB::get_one($query);
            cache::put('data/max_access_' . $_M['lang'], $result['access']);
            $_M['max_access'] = $result['access'];
        }
            $_M['module_parameters'] = cache::get('data/module_parameters_' . $_M['lang']);
            if (!$_M['module_parameters'] || defined('IN_ADMIN')) {
                $query = "SELECT * FROM {$_M['table']['parameter']} WHERE lang='{$_M['lang']}'  ORDER BY no_order ASC, id DESC";
                $data = DB::get_all($query);
                $parameter_group = array();
                foreach ($data as $val) {
                    $parameter_group[$val['module']][] = $val;
                }
                cache::put('data/module_parameters_' . $_M['lang'], $parameter_group);
                $_M['module_parameters'] = $parameter_group;
            }

            $_M['module_plist'] = cache::get('data/module_plist_' . $_M['lang']);
            if (!$_M['module_plist'] || defined('IN_ADMIN')) {
                $query = "SELECT * FROM {$_M['table']['plist']} WHERE lang='{$_M['lang']}'";
                $data = DB::get_all($query);
                $module_plist = array();
                foreach ($data as $val) {
                    $module_plist[$val['module']][] = $val;
                }
                cache::put('data/module_plist_' . $_M['lang'], $module_plist);
                $_M['module_plist'] = $module_plist;
            }


        $_M['class']['handle'] = load::sys_class('handle', 'new');
        $_M['class']['column_label'] = load::sys_class('label', 'new')->get('column');
    }
     ...
     * @param $lang 需要获取语言参数的语言
     * @param $site 获取语言参数位置，1:后台语言，2:前台语言
     */
    protected function load_word($lang, $site)
    {
        ...
        if (!file_exists($js_lang_cache)) {
            ...
            foreach ($result as $row) {
                $list[$row['name']] = self::replace_m(trim($row['value']));
            }
            $langstr = 'window.METLANG = ';
            $langstr .= jsonencode($list);
            file_put_contents($js_lang_cache, $langstr);
        }
    }
     ...
     */
    protected function ajaxReturn($data, $type = 'JSON', $json_option = 0, $callback = 'callback')
    {
        global $_M;
        switch (strtoupper($type)) {
            case 'JSON':
                if (isset($_M['plugin_error'])) {
                    $msg = implode(';', $_M['plugin_error']);
                    $this->error($msg);
                    return;
                }
