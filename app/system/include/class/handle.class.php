   ...
   */
    ...
    public function replace_list_page_url($url = '', $page = 1, $class = 0, $type = '')
    {
        ...
        if ($page == 1 && $class && !$_M['form']['search']) {
            $c = $_M['class']['column_label']->get_column_id($class);
            return $_M['class']['column_label']->handle->url_full($c, $type);
        } else {
            //tag搜索列表首页
    ...
    public function redirectUrl($data = array())
    {
        ...
        if (!$data['url']) return;
        if($_M['form']['search'] == 'tag' && strstr($_M['form']['content'] , '/')){
            // 搜索引擎各种拼接路径
            abort(404);
        }
        if (
        ...
        foreach ($items as &$v) {
            ...
            switch ($v[4]) {
                ...
                case 'smtp':
                    ...
                    break;
                case 'woff2':
                    $yes = @file_get_contents('../public/fonts/metinfo-icon1/metinfo-icon1.woff2');
                    break;
            }
