...
class tabledata
{
    ...
    public function getdata($table = '', $field = '*', $where = '', $order = '', $sql = '')
    {
        global $_M;
        /*获取表格ajax传递的参数*/
        $length = $_M['form']['length'] ? intval($_M['form']['length']) : 20;      //每页显示数量
        $start = $_M['form']['start'] ? intval($_M['form']['start']) : 0;        //读取数据的起点
    ...
    public function getDataAll($table = '', $field = '*', $where = '', $order = '')
    {
        global $_M;
        /*获取表格ajax传递的参数*/
        $this->length = $_M['form']['length'] ? intval($_M['form']['length']) : 20;      //每页显示数量
        $this->start = $_M['form']['start'] ? intval($_M['form']['start']) : 0;        //读取数据的起点
        $draw = $_M['form']['draw'] ? intval($_M['form']['draw']) : 1;           //累计执行次数，无作用但必须回传
        /*查询表*/
        $conds = '';
        if ($where) {
            $conds .= " WHERE {$where} ";
    ...
    public function rdata($rdata = array())
    {
        ...
        $this->rarray['draw'] = $this->rarray['draw'] ?  : $draw; //回传执行次数
        $this->rarray['recordsTotal'] = $this->rarray['recordsTotal'] ?: 0;                //回传总数量
        $this->rarray['recordsFiltered'] = $this->rarray['recordsFiltered'] ?: 0;           //回传筛选过的总数量，暂无作用，但必须回传
