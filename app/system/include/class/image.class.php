...
class image
{
    ...
    {
        global $_M;
        $this->default = str_replace('../', '', $_M['config']['met_agents_img']);
        if ($return) {
            $this->default = '';
        }
        if (!$image_path) {
    ...
    {
        if (strpos($this->image_path, '?')) {
            $path = explode('?', $this->thumb_path);
            $thumb_path = $path[0];
        } else {
    ...
    public function create_thumb()
    {
        ...
        if ($image['error']) {
            $image_path = $_M['url']['site'] . $this->default . $suf;
            return $image_path;
        }
     ...
     * @return string
     */
    public function getBase64Img($image_path)
    {
        $path = path_absolute($image_path);
        if(!file_exists($path)) return $image_path;
        ...
        if (isset($path_info['extension'])) {
            ...
            $type = isset($type_list[$ext]) ? $type_list[$ext] : "image/png";
        }else{
            $type = "image/png";
        }
