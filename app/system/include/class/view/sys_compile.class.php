...
class sys_compile extends compile
{
    ...
    public function parse_tag_static($type = 'css')
    {
        ...
        $new = $this->tem_path . 'cache/'.$this->skin_name.'.' . $type;
        ...
        if (($_M['form']['pageset'] && !defined('IN_ADMIN')) || !$has) {
            if (is_file($res)) {
                ...
                $sys_ver_str = str_replace('.', '_', $_M['config']['metcms_v']);
                if (SYS_TYPE == 'metinfo') {
                    $cms_name = "#文件名称：metinfo.{$type} #米拓企业建站系统 metinfo {$sys_ver_str}";
                    $cms_website = "https://www.metinfo.cn";
                }else{
                    $cms_name = "#文件名称：metinfo.{$type} #米拓网站管理系统 mituo_cms {$sys_ver_str}";
                    $cms_website = "https://www.mituo.cn";
                }
                $content .= "\n/*{$cms_name} #Copyright (C) 长沙米拓信息技术有限公司 ({$cms_website}). All rights reserved.*/";
    ...
    public function list_templates_config($is_css = '')
    {
        ...
        if (!$tem_config) {
            ...
            if ($_M['form']['pageset']) {
                $query = "SELECT name FROM {$_M['table']['templates']} WHERE lang = '{$_M['lang']}' AND no = '{$_M['config']['met_skin_user']}' AND name like 'tagshow_%'";
