...
final class met_view
{
     ...
     * @param null $cacheTime 缓存时长 秒
     * @param null $cachePath   缓存目录
     * @param string $contentType   文件类型
     * @param bool $show  输出编译文件
     * @return null|string
     */
    public function display($tplFile = null, $cacheTime = null, $cachePath = null, $contentType = "text/html", $show = true)
    {
        global $_M;
    ...
    private function compileInvalid()
    {
        ...
        if($_M['config']['debug']) return true; //debug
        if(!is_file($compileFile)) return true; //编译文件不存在
        if(filemtime($tplFile) > filemtime($compileFile)) return true;  //模板文件更新后
        if($cacheTime && filemtime($compileFile) > time() + $cacheTime) return true;    //编译文件缓存过期

        //return !is_file($compileFile) || $_M['config']['debug']|| (filemtime($tplFile) > filemtime($compileFile));
    }
