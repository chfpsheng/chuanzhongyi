...
", "\r\n", "\r", "\n", "\t", '@charset "utf-8";', "@charset 'utf-8';", '@charset "UTF-8";', "@charset 'UTF-8';"), '', $str);
    ...
    public function replace_public($path)
    {
        ...
        if(strstr($path, 'web-icons')
            ...
            ||strstr($path, 'formValidation.min.js')
            ||strstr($path, '}js/form.js')
            ||strstr($path, 'jquery-enplaceholder')
            ||strstr($path, 'jquery-appear.min.js')
            ||strstr($path, 'jquery.appear.min.js')
        ){
            $path='';
        }else{
            $replace = array(
                '{$metui_url1}vendor/' => PATH_PUBLIC_THIRD,
                '{$metui_url1}/vendor/' => PATH_PUBLIC_THIRD,
                '{$metui_url1}' => PATH_PUBLIC,
                '{$metui_url2}vendor/' => PATH_PUBLIC_THIRD,
                '{$metui_url2}/vendor/' => PATH_PUBLIC_THIRD,
                '{$metui_url2}js/Plugin/' => '',
                '{$metui_url2}/js/Plugin/' => '',
                '{$metui_url2}fonts/' => PATH_PUBLIC_THIRD.'fonts/',
                '{$metui_url2}' => PATH_PUBLIC,
                '{$metui_url3}plugin/' => PATH_PUBLIC_THIRD.'web/',
                '{$metui_url3}/plugin/' => PATH_PUBLIC_THIRD.'web/',
                '{$metui_url3}fonts/' => PATH_PUBLIC_THIRD.'fonts/',
                '{$metui_url3}/fonts/' => PATH_PUBLIC_THIRD.'fonts/',
            ...
            if(strstr($path, 'isotope.pkgd.min.js')&&!strstr($path, 'isotope/')){
                $path=str_replace('isotope.pkgd.min.js', 'isotope/isotope.pkgd.min.js', $path);
            }
            if(strstr($path, 'jquery.lazyload.min.js')&&!strstr($path, 'jquery-lazyload/')){
                $path=str_replace('jquery.lazyload.min.js', 'jquery-lazyload/jquery.lazyload.min.js', $path);
            }
            if(strstr($path, 'formvalidation')){
                $path=str_replace('formvalidation', 'formValidation', $path);
            }
            $init_path=$path;
            ...
            if(!is_file($path) && strstr($path,'public/third-party/web/')){
                $path=str_replace('public/third-party/web/','public/third-party/',$path);
            }
            if(!is_file($path) && strstr($path,'public/third-party/fonts/')){
                $path=str_replace('public/third-party/fonts/','public/fonts/',$path);
            }
        }
        return $path;
    }
    ...
    public function replace_sys_config()
    {
        ...
        if ($config['met_beian_info']) {
            $met_beian_info = $config['met_beian_info'];

            $img_url = "src=\"{$_M['url']['site']}public/images/gongan.png\"";
            $pattern = "/src=(\"|\')(\"|\')/";
            $met_beian_info = preg_replace($pattern, $img_url, $met_beian_info);

            $config['met_footother'] .= $met_beian_info;
        }
