...
<form method="POST" action="{$form_action}" data-submit-ajax='1'>
    <div class="clearfix">
        <if value="!$head_add_none || $dragsort_ok">
            <div class="float-left">
                <if value="!$head_add_none">
                    <button type="button" class="btn btn-outline-666 btn-content-list-add" data-toggle="modal" data-target=".{$data.module}-add-modal" data-modal-title="{$btn_add_text}" data-modal-size="xl" data-modal-url="{$data.module}/{$btn_add_file}/?c={$data.module}_admin&a=doadd&class1={$data.class1}&class2={$data.class2}&class3={$data.class3}" data-modal-fullheight="1">{$btn_add_text}</button>
                </if>
        ...
        <if value="!$head_search_none">
            <div class="input-group w-a <if value="!$head_add_none || $search_right">float-right<else/>float-left</if> <if value="$search_right"> content-list-search-right position-relative</if>">
