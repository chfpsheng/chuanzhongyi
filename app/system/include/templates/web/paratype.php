...
<div class="form-group row">
	<div class="met-form-lable blue-grey-600 col-md-3">{$val['name']}</div>
	<div class="col-md-9">
<!--
EOT;
		if(!$val['edit_ok']){
...
echo <<<EOT
-->
<div class="form-group met-upfile row">
	<div class="met-form-lable blue-grey-600 col-md-3">{$val['name']}</div>
	<div class="col-md-9">
		<input data-url="{$_M['url']['entrance']}?c=uploadify&m=include&lang={$lang}&a=doupfile&type=1" type="file" data-plugin='fileinput' name="info_{$val['id']}" value="{$value}" accept='image/*' {$wr_ok}>
	</div>
</div>
<!--
EOT;
...
EOT;
			...
			foreach($val['list'] as $key => $option){
				...
				if($option){
					$checked = ($value?$value==$option['id']:!$key)?'checked': '';
...
echo <<<EOT
-->
<div class="form-group row select-linkage" data-plugin='select-linkage'>
	<div class="met-form-lable blue-grey-600 col-md-3">{$val['name']}</div>
	<div class="col-md-9">
<!--
EOT;
