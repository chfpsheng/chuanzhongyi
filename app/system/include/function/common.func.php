 ...
 */
function met_safe_html($str)
{
    $str = (string)$str;
    $str = preg_replace('/<(script|style|iframe|object|embed|form|link|meta|svg|math)[^>]*>.*?<\/\1>/is', '', $str);
    $str = strip_tags($str, '<strong><b><em><i><u><s><p><br><h1><h2><h3><h4><h5><h6><ul><ol><li><blockquote><sub><sup><hr>');
    $str = preg_replace('/[\s\/]+(?:on[a-z]+|style)\s*=\s*("[^"]*"|\'[^\']*\'|[^\s>]+)/i', '', $str);
    $str = preg_replace('/\s(?:href|src)\s*=\s*(["\'])\s*(?:javascript|vbscript|data)\s*:/i', ' $1', $str);
...
function getbrowser()
{
    ...
    if ($browser != '') {
        return sqlinsert("{$browser} {$browser_ver}");
    }
    return '';
}
 ...
 * @return mixed
 */
function Base64Img($image_path)
{
    $image = load::sys_class('image', 'new');

    return $image->getBase64Img($image_path);
}
...
function strReplace($find, $replace, $array)
{
    if (is_array($array)) {
        ...
        foreach ($array as $key => $val) {
            if (is_array($val)) {
                $array[$key] = strReplace($find, $replace, $array[$key]);
            }
        }
    } else {
        $array = str_replace($find, $replace, $array);
...
function api_curl($url, $data = array(), $timeout = 60)
{
    ...
    $data['sys_type'] = defined("SYS_TYPE") ? SYS_TYPE : '';
