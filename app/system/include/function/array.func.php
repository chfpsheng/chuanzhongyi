 ...
 * @return array
 */
function arrayColumn($arr = array(), $column = '')
{
    if (!is_array($arr)) {
        return null;
    }
 ...
 * @return mixed
 */
function getTree(&$tree, $list, $pid = 0, $level = 0, $rel_feild = 'pid')
{
    foreach ($list as $key => $value) {
        if ($value[$rel_feild] == $pid) {
            $value['level'] = $level;
            $tree[] = $value;
            unset($list[$key]);
            getTree($tree, $list , $value['id'], $level + 1, $rel_feild);
        }
    }
