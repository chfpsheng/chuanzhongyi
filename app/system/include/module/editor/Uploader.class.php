...
class Uploader
{
    ...
    public function __construct($fileField, $config, $type = "upload")
    {
        ...
        $this->config = $config;
        $this->stateMap=array( //上传状态映射表，国际化用户需考虑此处数据的国际化
            ...
            "ERROR_IMG_PX_SIZE_EXCEED" => "图片尺寸超出网站限制 (宽度不超过{$config['imageCompressBorder'][0]}px，高度不超过{$config['imageCompressBorder'][1]}px)",
    ...
    private function upFile()
    {
        ...
        if ($imgattr && $this->config['imageCompressBorder']) {
            if ($imgattr[0] > $this->config['imageCompressBorder'][0] || $imgattr[1] > $this->config['imageCompressBorder'][1]) {
...
//        $img = file_get_contents($imgUrl);
        ...
        $path = $url_info['path'];
        $filename = pathinfo($path,PATHINFO_FILENAME) ?: random(8);

        $ext = end(explode('/', $heads['Content-Type']));
        $oriName = "{$filename}.{$ext}";
...
//        $this->oriName = $m ? $m[1]:"";
    ...
    {
        $extension = pathinfo($this->oriName, PATHINFO_EXTENSION) ?: "jpg";
...
//        return strtolower(strrchr($this->oriName, '.'));
    ...
    private function getFullName()
    {
        ...
        //替换随机字符串
        $randNum = rand(1, 1000000) . rand(1, 1000000) . rand(1, 1000000);
...
//        return $rootPath . $fullname;
    ...
    {
        return in_array(strtolower($this->getFileExt()), $this->config["allowFiles"]);
    }
