...
class weixinapi
{
    ...
    public function getToken()
    {
        ...
        if($info['errcode']){
            ...
            $this->error[] = $error;
            file_put_contents(__DIR__ . '/wx_error.log', var_export($error, true));
            return false;
        }
    ...
    public function QRcode($info = '')
    {
        ...
        $data = array(
            ...
            'action_info' => array(
                'scene'=>array(
                    'scene_str' => $info,   //场景值ID（字符串形式的ID），字符串类型，长度限制为1到64
                )
            ),
        );
...
//        file_put_contents(__DIR__ . '/wx_msg_res.txt', var_export($res, true));
    ...
    public function getAdminBindInfo($info){
        global $_M;
        $info['met_weixin_install'] = 0;
        $info['met_weixin_config'] = 0;
        $info['met_weixin_openid'] = $info['openid'];
        
        if (file_exists(PATH_ALL_APP.'met_weixin')){
            $info['met_weixin_install'] = 1;
        }
        
        if ($_M['config']['met_weixin_gz_appid'] != "" && $_M['config']['met_weixin_gz_appsecret'] != ""){
            $info['met_weixin_config'] = 1;
        }

        if($info['met_weixin_openid'] == ''){
            if ($info['met_weixin_config']){
                
            $code = "adminbind&".$info['admin_pass'];
            cache::put("weixin/".$code,$info['id'],'txt');
            
            $imgUrl = $this->QRcode($code);
                if($imgUrl){
                    $info['qrcode'] = $imgUrl;
                }
            }
    ...
    public function getAdminWxLoginImg($login_code){
        global $_M;
        
        if ($_M['config']['met_weixin_gz_appid'] != "" && $_M['config']['met_weixin_gz_appsecret'] != ""){
            $code = "adminlogin&".$login_code;
            cache::put("weixin/".$code,$login_code,'txt');
            $imgUrl = $this->QRcode($code);
            if($imgUrl){
                return $imgUrl;
            }
