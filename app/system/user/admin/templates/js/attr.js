...
/*                 M.component.formWidget({
                ...
                `<button class="btn btn-light btn-delete" data-head_title data-id="${val.id}">${METLANG.delete}</button>
  ...
  function delAll() {
    $(document).on('click', '#user-attr-table .btn-delete-all', function(e) {
      ...
      if ($ids.length === 0) {
        alertify.alert(METLANG.js23,'','','tips');
        return
      }
      $ids.each(function() {
        ids.push($(this).val())
      ...
      callback: function() {
        ...
        options.map(item => {
          html += `<tr>
            ...
            <td><button type="button" class="btn btn-light btn-para-delete">${METLANG.delete}</button></td></tr>`
        })
