            ...
            that.data &&
              $.each(that.data, function(index, val) {
               ...
               >${METLANG.editor}</button>
                <button class="btn btn-delete ml-2" data-head_title data-id="${val.id}">${METLANG.delete}</button>`
                ]
                newData.push(list)
              })
  ...
  function getUserGroup() {
    ...
    $.ajax({
      ...
      success: function(result) {
        ...
        result.data.group_list && result.data.group_list.length && result.data.group_list.map(item => {
