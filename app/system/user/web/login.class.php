...
class login extends userweb
{
    ...
    public function doindex()
    {
        ...
        $sys_session = load::sys_class('session', 'new');
        $check_times = $sys_session->get("logineorrorlength");
        ...
        if ($_GET['gourl']) {
            setcookie("referer", urldecode($_GET['gourl']));
        }

        if ($_M['config']['met_memberlogin_code']) {
            $this->input['captcha'] = 1;
            $this->input['random'] = random(4, 1);
        } elseif ($_M['config']['met_captcha_open']) {
            $this->input['captcha'] = 0;
        } elseif ($check_times > 3) {//登录验证错误超过3次，开启图形验证
            $this->input['captcha'] = 1;
            $this->input['random'] = random(4, 1);
        }
    ...
    {
        global $_M;
        $username = authcode($_M['form']['username']);
        $password = authcode($_M['form']['password']);
        //验证码
        $sys_session = load::sys_class('session', 'new');
        $check_times = $sys_session->get('logineorrorlength') ?: 0;
        $locked = $this->userclass->checkUserLock($username);

        if ($_M['config']['met_memberlogin_code']) {//系统验证码
            $res = load::sys_class('pin', 'new')->check_pin($_M['form']['code'], $_M['form']['random']);
            if (!$res) okinfo($_M['url']['user_home'], $_M['word']['membercode']);
            ...
            if($locked) $sys_session->set("logineorrorlength", 4);
            $res = load::sys_class('pin', 'new')->check_pin($_M['form']['code'], $_M['form']['random']);
            if (!$res) okinfo($_M['url']['user_home'], $_M['word']['membercode']);
        }
        ++$check_times;
        $sys_session->set("logineorrorlength", $check_times);

        //验证用户登录信息
        $res = $this->login($username, $password);
        if (!$res) okinfo($_M['url']['login'], $this->error);
        $sys_session->del("logineorrorlength");
...
//        $paygroup = load::mod_class('user/sys_group', 'new');
        ...
        if (!$user){
            logs::addLog('webuser', 'loginbypassword', 'failed', $username);
            $this->error = $_M['word']['membererror1']; //用户账号密码错误
            return false;
        }

        if (!$user['valid']){
            $this->error = $_M['word']['membererror6']; //用户未激活
            return false;
        }

        logs::addLog('webuser', 'loginbypassword', 'success', $username);
        $this->userclass->setLoginRecord($user);
        return $user;
    }
    ...
    public function doother_login()
    {
        ...
        if ($user) {
            $res=$this->login($user['username'], md5($user['password']), 'md5');
            if($res){
                okinfo($_M['url']['login']);
            }
        } else {
    ...
    public function docheckwxlogin()
    {
        ...
        if ($check_res) {
            if ($check_res['register'] != 1) {//已注册用户
                ...
                $sys_user = $this->userclass->login_by_password($user['username'], md5($user['password']), 'md5');
                $this->userclass->setLoginRecord($sys_user);
