...
class index extends admin
{
    ...
    public function doSqliteBak()
    {
        global $_M;
        $db_path = PATH_WEB . $_M['config']['db_name'];
        $filename = 'sqlite_' . date('Ymd') . '_' . random(6) .  '_1.db';
        $backup = PATH_WEB . ADMIN_FILE . '/databack/sqlite/';
        if (!file_exists($backup)) {
            mkdir($backup, 0777, true);
        }
        $bakfile = $backup . $filename;
        copy($db_path, $bakfile);
        $this->success('备份成功');
    }
    ...
    public function dogetsql()
    {
        ...
        for ($i = $tableid - 1; $i < $tablenumber && strlen($sqldump) < $sizelimit * 1000; ++$i) {
            $sqldump .= $this->sql_dumptable($tables[$i], $startfrom, strlen($sqldump));
            $startfrom = 0;
        }
     ...
     * @return string
     */
    public function sql_dumptable($table = '', $startfrom = 0, $currsize = 0)
    {
        global $_M;
        if (!$table) return '';
        ...
        if (!$startfrom) {
            ...
            switch (strtolower($_M['config']['db_type'])) {
                case 'mysql':
                    $query = "SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = '{$_M['config']['con_db_name']}' AND TABLE_NAME = '{$table}'";
                    $table_exists = DB::get_one($query);
                    if (!$table_exists) {
                        return $tabledump;
                    }

                    $res = DB::get_one("SHOW CREATE TABLE {$table}");
                    $create_str = $res['Create Table'] ?: "";
                    $tabledump .= str_replace(strtolower($table), $table, $create_str) . ";\n\n";
                    break;
                case "sqlite":
    ...
    public function dopackupload()
    {
        ...
        self::Recursive($zip, $upload_path);
        $zip->close();
    ...
    public function doallfile()
    {
        ...
        //备份忽略文件
        $skip_1 = scan_dir($web_back_path);
        $skip_2 = scan_dir(PATH_WEB . 'cache');
        $skip_list = array_merge((array)$skip_1, (array)$skip_2);

        //Create zip
        $zip = new ZipArchive();
        $zip->open($web_zip, ZIPARCHIVE::CREATE | ZIPARCHIVE::OVERWRITE);
        self::Recursive($zip, $web_path, '', $skip_list);
        $zip->close();
     ...
     */
    protected function Recursive(ZipArchive $zip, $target_dir,  $skip = '', $skip_list = array())
    {
        $skip = $skip ?: PATH_WEB;
        ...
        if (
            ...
            foreach ($files as $file) {
                ...
                if (!$file->isDir()) {
                    ...
                    $relativePath = str_replace($skip, '', $filePath);
                    if (!in_array($relativePath, $skip_list)) {
                        $zip->addFile($filePath, $relativePath);
                    }
                }
            }
        } else {
            $handler = opendir($target_dir);
            while ($filename = readdir($handler)) {
                if (in_array($filename, array('.', '..'))) {
                    continue;
                }

                $realPath = $target_dir . '/' . $filename;
                $relativePath = str_replace($skip, '', $realPath);
                if (is_dir($realPath)) {
                    if ($skip != $realPath) {
                        $zip->addEmptyDir($relativePath);
                    }
                    self::Recursive($zip, $realPath, $skip, $skip_list);
                } else {
                    //忽略文件列表
                    if (!in_array($relativePath, $skip_list)) {
    ...
    public function dogetfileattr()
    {
        ...
        $sqlitefiles = glob(PATH_WEB . ADMIN_FILE . '/databack/sqlite/*.db');
        $files = array_merge($sqlfiles, $sqlitefiles);
        if (is_array($files)) {
            $prepre = '';
            $info = $infos = array();
            foreach ($files as $id => $sqlfile) {
                preg_match("/(.*_)([0-9]+)\.(sql|db)/i", basename($sqlfile), $num);
                $info['filename'] = basename($sqlfile);
                $filesize = round(filesize($sqlfile) / (1024 * 1024), 2);
                $info['filesize'] = strval($filesize);
                $info['maketime'] = date('Y-m-d H:i:s', filemtime($sqlfile));
                $info['pre'] = $num[1];
                $info['number'] = $num[2];
                $info['type'] = 'sql';
                if (strpos($sqlfile, '.db') !== false) {
                    $info['type'] = 'sqlite';
                }
        ...
        foreach ($infos as $key => $val) {
            ...
            $filesize = strval(round($val['filesize'], 2));
            ...
            $infos[$key]['filesize'] = $filesize;
            //检测导入数据版本
            if (version_compare($import_version, $this->ver_allow, '<') && $val['type'] != 'sqlite') {
                $infos[$key]['error'] = '2';
            ...
            if ($info_num - 1 != $val['number'] && $val['type'] != 'sqlite') {
                $infos[$key]['error'] = '1';
    ...
    public function doimport()
    {
        global $_M;
        if ($_M['config']['db_type'] == 'sqlite') {
            return $this->error('当前数据库类型不支持导入');
            // $prefix = $_M['form']['pre'];
            // $dbfile = PATH_WEB . ADMIN_FILE .'/databack/sqlite/'.$prefix.'1.db';
            // if(!file_exists($dbfile)){
            //     $this->error('备份文件不存在');
            // }
            // $old = PATH_WEB.$_M['config']['db_name'];
            // copy($old, PATH_WEB . ADMIN_FILE .'/databack/sqlite/sqlite_' . date('Ymd') . '_' . random(6) .  '_1.db');
            // rename($dbfile,$old);
            // $result['status'] = 1;
    ...
    public function dosql_execute()
    {
        ...
        $version = $_M['form']['version'] ?: $_M['config']['metcms_v'];
        ...
        if (file_exists($filepath)) {
            ...
            $split = $this->dosql_split($sql_data);
            $info = $split['info'];
            $sqls = $transfer->getQuery($sql_data);
            $infos = explode('#', $info);
            ...
            if (is_array($sqls)) {
                ...
                foreach ($sqls as $sql) {
                    //不更新后台看栏目表
                    if (strstr($sql, "{$tablepre}admin_column") || strstr($sql, "{$tablepre}admin_permissions") || strstr($sql, "{$tablepre}admin_menus")) {
                        continue;
                    }
                    //sql关键词过滤
                    ...
                    switch (strtolower($_M['config']['db_type'])) {
                        ...
                        case 'sqlite':
                            ...
                            if (stristr($sql, 'CREATE TABLE')) { //创表语句替换
                    ...
                    if (preg_match($pattern, $sql)) {
                        $sql = preg_replace_callback($pattern, function ($match) {
                            if ($match) {
                                ...
                                $string = str_ireplace('dumpfile', "\dump\file", $string);
            ...
            if ($version != $old_version) {
                ...
                if (version_compare($old_version, $this->ver, '<')) {
                    //更新语言
                    $update_database->update_language($version);
                    $update_database->update79->defaultDate();
                    $update_database->update79->setRolesPermissions();
                    $update_database->update79->setMemberPermissions();
                }
            }
    ...
    protected function checkKeyWords($sql = '')
    {
        ...
        foreach ($key_words as $word) {
            if (stristr(strtolower($sql), $word)) {
                return false;
            }
        }
        return true;
    ...
    public function dodelete()
    {
        ...
        if (strstr($prefix, 'sqlite_')) {
            unlink(PATH_WEB . ADMIN_FILE .'/databack/sqlite/'.$prefix.'1.db');
        } else {
            $ext = '.sql';
    ...
    public function dounzip_upload()
    {
        ...
        if ($zip->open($zipname) === true) {
            ...
            $this->success('', $_M['word']['webupate3']);
        } else {
            //写日志
    ...
    public function dodelete_zip()
    {
        ...
        $this->error($_M['word']['setdbNotExist']);
    }
    ...
    public function doUploadDataback()
    {
        ...
        if ($back['error']) {
            ...
            if (file_exists($f_paht)) {
                ...
                $filesize = round($back['size'] / 1024, 2);
                $info['filesize'] = strval($filesize);
