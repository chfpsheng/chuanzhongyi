...
(function () {
				...
				ajax: {
					dataSrc: function (result) {
						...
						$.each(that.langData, function (index, val) {
							let list = [
								val.no_order,
								`<div class="d-flex align-items-center text-wrap">${val.name}${val.met_admin_type == val.mark ? `<span class="badge font-weight-normal badge-pill badge-light-danger ml-2">${M.synchronous=='cn'?'默认语言':'default language'}</span>` : ''}</div>`,
								...
								<button class="btn btn-light-success m-1 btn-search-edit"
									...
									data-modal-tablerefresh="#lang-admin-table"
									data-modal-title="${METLANG.langwebeditor}">${METLANG.langwebeditor}
								</button>
								${langset != val.mark?`<button
									type="button"
									class="btn btn-light-danger m-1 btn-langadmin-delete" data-head_title
								...
								<button class="btn btn-default m-1 mt-2 btn-replace"
									...
									data-modal-tablerefresh="#lang-admin-table">${METLANG.language_batchreplace_v6}
								</button>
								<button type="button" class="btn btn-default m-1 mt-2 btn-export-pack"
									data-url="${that.own_name}c=language_general&a=doExportPack&site=admin&editor=${val.mark}">
									${METLANG.language_outputlang_v6}
								</button>
								${sysMark.indexOf(val.mark) > -1?
								`<button class="btn btn-default m-1 mt-2 btn-sync"
									type="button"
									data-index="${index}"
	...
	$(document).on('click', '#lang-admin-table .btn-export-pack', function (e) {
