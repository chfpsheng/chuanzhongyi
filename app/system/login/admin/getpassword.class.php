...
class getpassword extends admin
{
    ...
    protected function getPasswordByEmail($form = array())
    {
        ...
        if (!is_email($admin_email)) $this->error($_M['word']['password7']);

        //仅限超级管理员
        $query = "SELECT * FROM {$_M['table']['admin_table']} WHERE admin_email='{$admin_email}' AND admin_type='metinfo'";
        $admin_member = DB::get_one($query);

        if (!$admin_member) $this->error($_M['word']['password14']);

        //拼装加密数据
        $x = md5($admin_member['admin_id'] . '+' . $admin_member['admin_pass']); //验证信息加密
        $outime = 3600 * 24 * 3;
        $string = authcode($admin_member['admin_id'] . "." . $x, 'ENCODE', $_M['config']['met_webkeys'], $outime);
        $string = urlencode($string);
        $mailurl = "{$_M['url']['admin_site']}index.php?n=login&c=getpassword&a=doresetpass&langset={$langset}&abt_type=2&p={$string}";


        //拼装邮件内容
        $admin_name = $admin_member['admin_id'];
        $to = $admin_email;
        ...
        if ($sendMail) {
            //成功！！
            $msg = $_M['word']['getTip3'] . $_M['word']['memberEmail'] . '：' . $admin_email;
            logs::addAdminLog('admin_getpassword', 'send_email', 'ok', 'getPasswordByEmail');
            $this->success('', $msg);
        } else {
            $msg = $_M['word']['password30'];
    ...
    {
        global $_M;
        $admin_mobile = $form['admin_id'];

        //$sql = "SELECT * FROM {$_M['table']['admin_table']} WHERE admin_id='{$admin_id}' and usertype='3'";
        $sql = "SELECT * FROM {$_M['table']['admin_table']} WHERE admin_mobile = '{$admin_mobile}' AND admin_type='metinfo'";
        $admin_member = DB::get_one($sql);
        if (!$admin_member) $this->error($_M['word']['password8']);

        //短信验证码
        $code = random(6, 1);    //验证码
        $nber = random(4, 5);    //信息编号
        $cnde = $code . '-' . $nber . '-' . $admin_member['admin_id'];
//        file_put_contents(__DIR__ . '/cc.log', $cnde);

        //发送短信
        //检测短信是否可用
        $checkBySMS = self::checkFindBySMS();
        if (!$checkBySMS) $this->error($_M['word']['password13']);
        ##您请求重新设置密码，验证码 859055 [http://localhost/]
        ...
        $smsok = $sms_obj->sendsms($admin_member['admin_mobile'], $message, 3);

        if ($smsok['status'] !== 200) {
            logs::addAdminLog('admin_getpassword', 'sendsms', 'false', 'getPasswordByMobile');
            $this->error($_M['word']['password13'], "短信发送失败");
        }

        //写入数据库
        $session = load::sys_class('session', 'new');
        $session->set('admin_find_pass', $cnde);

        //拼装加密数据
        $x = md5($admin_member['admin_id'] . '+' . $admin_member['admin_pass']); //验证信息加密
        $outime = 3600 * 24 * 3;
        $string = authcode($admin_member['admin_id'] . "." . $x . '.' . $nber, 'ENCODE', $_M['config']['met_webkeys'], $outime);
        $string = urlencode($string);
        $mobile = substr($admin_member['admin_mobile'], 0, 3) . '****' . substr($admin_member['admin_mobile'], 7, 10);
    ...
    public function doCodeCheckSave()
    {
        ...
        $session = load::sys_class('session', 'new');
        $codeok = $session->get('admin_find_pass');
    ...
    {
        global $_M;
        if (!$form['p'] || !$form['code']) $this->error($_M['word']['dataerror']);
