/**
 * 少儿中医模块后台：「所属医馆」下拉框增加顶部搜索框。
 *
 * 适配 MetInfo 后台自动注入的 met-select 皮肤（与 activity.js、doctor.js 逻辑一致）：
 ...
 *
 * met-select 的菜单 HTML 是 wrap 时一次性生成的静态结构，因此采用「在下拉菜单顶部插入
 * 搜索框 + 按文本过滤 d-none」的方式实现筛选。
 *
 * 加载方式：本文件位于 app/system/shaoer/admin/templates/js/shaoer.js，
 * 由后台 foot 自动加载到少儿中医列表页，通过 setInterval 持续扫描，
 * 确保弹窗 innerHTML 注入的表单也能被处理。
 *
 * 注意：app/system/activity/admin/templates/js/activity.js 与本文件逻辑相同，
 * 如需调整搜索行为请同步修改两个文件。
 */
(function () {
    'use strict';
