...
class search_op
{
    ...
    public function insert_search_info($module, $listid)
    {
        ...
        if ($mid == 1) {
            $list = $_M['class']['column_label']->get_column_id($listid);
            ...
            $search['urlinfo'] = array(
                1 => $_M['class']['column_label']->handle->get_content_url($list, 1),
                2 => $_M['class']['column_label']->handle->get_content_url($list, 2),
                3 => $_M['class']['column_label']->handle->get_content_url($list, 3),
            );
            $search['top_ok'] = '';
            $search['com_ok'] = '';
            $search['update_time'] = '';
