...
class base_handle extends handle
{
    ...
    public function construct($contents_page_name)
    {
        ...
        $this->column_label = $_M['class']['column_label'];
        $this->publish_list = load::mod_class("admin/admin_database", 'new')->getAllPublish();
    }

    /**********数据处理************/
    /**
     * 处理list数组
     * @param  array $list 处理列表数组
     * @return array         处理过后数组
     */
    public function para_handle($list=array(), $para = 0)
    {
        global $_M;
    ...
    public function one_para_handle($content = array())
    {
        ...
        if ($content) {
            ...
            if ($content['video']) $content['video'] = $this->replace_relative_url($content['video']);
            ...
            if(!strstr($content['description'],'<br>') && strstr($content['description'],"\n")){
                $content['description'] =  str_replace("\n",'<br>',$content['description']);
            }

            $c = $this->column_label->get_column_id($content['class1']);
            $content['module'] = $c['module'];
            //发布人 老模板兼容
            $publisher = self::getPublish($content['issue']);
            $content['issue'] = $content['publisher'] ?: $publisher;
            //图片列表处理
     ...
     * @return mixed|string
     */
private function getPublish($admin_id)
{
    // 若 $admin_id 未设置，直接返回空字符串
    if (empty($admin_id)) {
        return '';
    }

    // 若缓存中已存在该管理员名称，直接返回
    if (isset($this->publish_list[$admin_id])) {
        return $this->publish_list[$admin_id];
    }
    return '';
}
    ...
    {
        $c = $this->column_label->get_column_id($id);
        $class = $this->column_label->get_class123_no_reclass($id);
        ...
        if ($content['links']) {
            if ($content['access'] != 0) {
                if ($content['access']) {
                    ...
                    $links = "{$_M['url']['entrance']}?m=include&c=access&a=dojump&lang={$_M['lang']}&url={$url}&groupid={$groupid}";
                }
                return $links;
            } else {
    ...
    public function get_list_page_url($column_class1, $column_class2, $column_class3, $column_file, $module_name, $filename, $lang, $type = 1)
    {
        ...
        if ($_M['form']['search'] == 'tag') {
            // 标签搜索的分页
            $data = array('cid' => $column_class1, 'module' => $this->file_to_mod($module_name));
