...
class  feedback_database extends base_database
{
    ...
    {
        global $_M;
        $query = "SELECT c1.*, p.related 
                 FROM {$_M['table']['config']} c1 
                 LEFT JOIN {$_M['table']['config']} c2 
                    ON c1.columnid = c2.columnid 
                    AND c2.name = 'met_fd_related'
                 LEFT JOIN {$_M['table']['parameter']} p 
                    ON c2.value = p.id 
                 WHERE c1.name = 'met_fd_inquiry' 
                    AND c1.value = '1'
                    AND c1.lang = '{$_M['lang']}'";
        $met_fd_inquiry = DB::get_one($query);
       
        if ($met_fd_inquiry && $met_fd_inquiry['related']) {
            $met_fd_inquiry['value'] = $met_fd_inquiry['related'];
        }
