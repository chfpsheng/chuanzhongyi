...
class feedback_admin extends message_admin
{
    ...
    public function dojson_list()
    {
        ...
        $list = $this->getJsonList($class1, $class2, $class3, $keyword, $search_type, $checkok ,$orderby_hits, $orderby_updatetime);
        $redata['data'] = $list;
     ...
     * 反馈分页数据
     */
    public function getJsonList($class1 = '', $class2 = '', $class3 = '', $keyword = '', $search_type = '', $checkok = '', $orderby_hits = '', $orderby_updatetime = '')
    {
        global $_M;
        if ($class3) {
            $classnow = $class3;
        ...
        foreach ($_M['form'] as $key => $row) {
            if (strstr($key, 'para_') && $row) {
                $para_id = intval(str_replace('para_', '', $key));
                if ($para_id) $para_fields[$para_id] = $row;
            }
        }
        ...
        $order = '  addtime desc';
        $feedbacklist = $this->pagelist($where, $order);
    ...
    public function doview()
    {
        ...
        if ($feedback) {
            ...
            $feedback['fromurl'] = htmlspecialchars($feedback['fromurl'], ENT_QUOTES, 'UTF-8');
        }
    ...
    public function dosyset()
    {
        ...
        $redata['list']['met_fd_email'] = self::get_config_field(9, $conlum_configs['met_fd_email'], $classnow);
        $redata['list']['met_fd_class'] = self::get_config_field(array(2, 6), $conlum_configs['met_fd_class'], $classnow);
        $redata['list']['met_fd_sms_tell'] = self::get_config_field(8, $conlum_configs['met_fd_sms_tell'], $classnow);
        $redata['list']['met_fd_related'] = self::get_config_field(array(2, 4, 6), $conlum_configs['met_fd_related'], $classnow);
    ...
    public function dosaveinc()
    {
        ...
        $config_op = load::mod_class('config/config_op', 'new');
        foreach ($this->module_config as $name) {
    ...
    public function get_config_field($type = '', $value = '', $classnow = '')
    {
        ...
        $paralist = load::mod_class('parameter/parameter_label', 'new')->get_parameter($this->module, $class123['class1']['id'], $class123['class2']['id'], $class123['class3']['id']);
        $list = array();
        $unll = $_M['word']['please_choose'] ?: '--';
