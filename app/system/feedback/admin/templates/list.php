...
<include file="pub/content_list/head"/>
				...
				<th data-table-columnclass="text-center" width="50">IP</th>
				<th width="100">{$word.operate}</th>
			</tr>
		</thead>
		<tbody>
			<?php $colspan=6+count($data['showcol']); ?>
		...
		<include file="pub/content_list/tfoot_common"/>
					<div class="float-right">
						...
						<a href="{$url.own_name}c=feedback_admin&a=doexport&class1={$data.class1}&class2={$data.class2}&class3={$data.class3}&met_fd_export=-1" data-href="{$url.own_name}c=feedback_admin&a=doexport&class1={$data.class1}&class2={$data.class2}&class3={$data.class3}&met_fd_export=-1" class="btn btn-outline-primary ml-2 btn-feedback-export">{$word.feedbackTip2}</a>
					</div>
				</th>
			</tr>
		</tfoot>
