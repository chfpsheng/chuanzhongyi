...
<header class='pageset-head bg-dark' style="height: 50px;">
	<div class="container-fluid h-100 position-relative">
		...
		<if value="is_mobile()">
		<button class="btn btn-outline-light py-1 btn-sm btn-block mt-2 btn-pageset-mobile-menu">{$word.top_menu}</button>
		</if>
		...
		<div class="container h-100 pageset-head-nav">
			<div class="row h-100 navbar p-0">
				<div>
	            	<a href class="btn btn-outline-light py-1 border-none pageset-view" title="{$word.uisetTips4}" target='_blank'>{$word.preview}</a>
                    <a href='javascript:;' class="btn btn-outline-light py-1 border-none {$headnav_ml} pageset-other-config" data-config-url='{$url.own_form}a=doget_page_config' data-form_action="doset_page_config" title="{$word.uisetTips5}">{$word.uisetTips6}</a>
				</div>
				<div class="float-right">
					<div class="btn-group {$headnav_ml}">
						<button class="btn btn-outline-light py-1 border-none dropdown-toggle" type="button" data-toggle="dropdown">{$word.indexbbs}</button>
						<ul class="dropdown-menu dropdown-menu-right mt-2">
							<if value="$c['met_agents_metmsg']">
							<a href="{$c.help_url}" class='dropdown-item px-3' target='_blank'>{$word.help_manual}</a>
							<a href="{$c.edu_url}" class='dropdown-item px-3' target='_blank'>{$word.extension_school}</a>
							<a href="{$c.kf_url}" class='dropdown-item px-3' target='_blank'>{$word.online_work_order}</a>
							</if>
					...
					<a href='{$url.site_admin}' class="btn btn-outline-light py-1 border-none {$headnav_ml}" target='_blank'>{$word.oldBackstage}</a>
					<?php $lang_name = $_M['langlist']['web'][$_M['lang']]['name']; ?>
					<div class="btn-group {$headnav_ml}">
		                <button class="btn btn-outline-light py-1 border-none dropdown-toggle" type="button" data-toggle="dropdown">{$lang_name}</button>
		                <ul class="dropdown-menu dropdown-menu-right mt-2 metadmin-head-langlist">
	                        ...
	                        <li class="px-3 py-1"><a href="javascript:;" class="btn btn-outline-666" data-toggle="modal" data-target=".pageset-nav-modal" data-url="language"><i class="fa fa-plus"></i> {$word.added}{$word.langweb}</a></li>
		                </ul>
		            </div>
					<if value="$c['met_agents_metmsg']">
					<a href='javascript:;' class="btn btn-outline-light py-1 border-none position-relative {$headnav_ml}" data-toggle="modal" data-target=".pageset-nav-modal" data-url='system/news' title="{$word.sysMessage}">
						<i class="fa fa-bell-o"></i>
						<span class="sys-news-count position-absolute"></span>
					</a>
					</if>
					<div class="btn-group {$headnav_ml}">
						<button type="button" class="btn btn-outline-light py-1 border-none dropdown-toggle pageset-head-user" data-toggle="dropdown"><span class='text-truncate d-inline-block position-relative' style="max-width: 100px;top: 3px;">{$data.admin_member.admin_id}</span></button>
						<ul class="dropdown-menu dropdown-menu-right mt-2">
							<a href='javascript:;' class='dropdown-item px-3' data-toggle="modal" data-target=".pageset-nav-modal" data-url="admin/admin_sys/?n=admin&c=index&a=doInfo" title="{$word.indexadminname}">{$word.modify_information}</a>
							<a href="{$url.site_admin}?n=login&c=login&a=dologinout" class='dropdown-item px-3'>{$word.indexloginout}</a>
						</ul>
					</div>
				</div>
			</div>
...
<div class="text-center mb-0 bg-grey alert pageset-tips">
    ...
    <div>
        <button type='button' class="btn btn-default no-prompt" data-url="{$url.site_admin}?n=index&c=index&a=doNoPrompt" data-dismiss="alert">{$word.nohint}</button>
							...
							<div class="uiset-guide-box bg-white p-4">
								...
								<div class="p-3">
									...
									<div class="uiset-guide-body h6 mb-0">
										<p>你好 <span class="text-primary">{$data.admin_member.admin_id}</span>：</p>
