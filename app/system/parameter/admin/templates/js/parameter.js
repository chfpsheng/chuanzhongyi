...
(function(){
			...
			setTimeout(()=>{
				formSaveCallback(that.obj.find('form').attr('data-validate_order'),{
					true_fun:function(result){
						$.inArray(that.data.module, ['message','feedback','job'])>=0 && $(that.obj.parents('.content-show-item').find('.met-headtab a[data-url^="'+that.data.module+'/set/"]').attr('href')).removeAttr('data-loaded');
						result.data.html_res && contentCreateHtml(result.data.html_res);
					}
				});
			},500)
		});
					...
					if(result.data){
						$.each(result.data, function(index, val) {
							var options_show=$.inArray(parseInt(val.type), [2,4,6])>=0?1:0,
								...
								if($.inArray(that.data.module, ['news','product','img','download'])<0){
								...
								if($.inArray(that.data.module, ['news','product','img','download'])<0) item=item.concat([M.component.formWidget({
									name:'wr_ok-'+val.id,
									type:'select',
									value:val.wr_ok,
								...
								item=item.concat([
									'<button type="button" class="btn px-2 btn-light-primary mr-1 btn-parameter-setoptions" data-toggle="modal" data-target=".parameter-options-modal" data-modal-title="'+METLANG.listTitle+'" '+(options_show?'':'hidden')+'>'+METLANG.listTitle+'</button>'
									+M.component.formWidget({
									...
									}):M.component.formWidget('related-'+val.id,val.related))
								]);
							data.push(item);
						});
					}
    ...
    // 保存参数-特殊处理
    $(document).on('click', parameter_list+' .btn[type="submit"]',function(event) {
		var $need_check=$(this).parents('table').find('tbody tr input[name*="related-"][value!=""]');
    	$need_check.parents('tr').find('.checkall-item').prop('checked',true);
		var allid=$(this).parents('form').find('[name="allid"]').val(),
			allid_list=allid.split(',');
		$need_check.each(function(){
			$(this).val('');
			var id=$(this).attr('name').split('-')[1];
			if(!allid_list.includes(id)) allid+=`${allid?',':''}${id}`;
		});
		$(this).parents('form').find('[name="allid"]').val(allid);
    });
    // 参数属性框-html
    var parameter_options_tr=function(data){
	    	...
	    	return '<tr>'
	    				...
	    				+'<td><button type="button" class="btn btn-light" table-del>'+METLANG.delete+'</button></td>'
	    			+'</tr>'
	    };
	    	...
	    	+'<tfoot>'
	    		+'<tr>'
	    			...
	    			+'<th colspan="2">'
	    				...
	    				+'<button type="button" class="btn btn-light" table-del>'+METLANG.delete+'</button>'
	    				+'<button type="button" class="btn btn-outline-666 ml-1" table-addlist data-nocancel="1">'+METLANG.add+'</button>'
    ...
    $(document).on('click', '.parameter-options-modal [data-ok]', function(event) {
    	...
    	if(!$modal.find('.has-danger').length){
    		var options=[];
    		$('#parameter-options-list tbody tr').filter(function(){
				return $('.checkall-item',this).length;
