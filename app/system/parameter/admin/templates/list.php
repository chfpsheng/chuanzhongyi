						...
						<tr>
							...
							<td class="text-center">
								<select name="class" data-checked="{$data.class1}-{$data.class2}-{$data.class3}" class="form-control w-a d-inline-block">
									<list data="$data['class_options']" name="$v" num="150">
									<option value="{$v.val}">{$v.name}</option>
									</list>
								</select>
							</td>
							...
							<td>
								<button type="button" class="btn px-1 btn-light-primary btn-parameter-setoptions" data-toggle="modal" data-target=".parameter-options-modal" data-modal-title="{$word.listTitle}" hidden>{$word.listTitle}</button>
