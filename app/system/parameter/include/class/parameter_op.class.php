...
class parameter_op
{
    ...
    public function insert($listid = '', $module = '', $paras = array())
    {
        ...
        foreach ($list as $key => $val) {
            ...
            $this->parameter_database->insert_list($listid, $key, $val, $paramenter_name, $module);
        }
        return true;
    ...
    public function update($listid = '', $module = '', $paras = array())
    {
        ...
        foreach ($list as $key => $val) {
            ...
            $paramenter = $this->parameter_database->get_parameter_by_id($key);
            $paramenter_name = $paramenter['name'];
            $this->parameter_database->update_list($listid, $key, $val, $paramenter_name, $module);
        }
        return true;
     ...
     * @param string $class2
     * @param string $class3
     * @param integer $hid  //历史记录id
     * @return array
     */
    public function paratem($listid = '', $module = '', $class1 = '', $class2 = '', $class3 = '' ,$hid = 0)
    {
        global $_M;
...
//                $query = "SELECT * FROM {$plist_table} WHERE listid = '{$listid}' AND paraid='{$para['id']}' AND module={$module} AND lang = '{$_M['lang']}'";
//                $para_value = DB::get_one($query);
//                $values = $para_value['info'];
//            }

            if ($hid) {//获取参数历史记录
                $para_value = load::mod_class('history/history_op', 'new')->getPlistHistory($listid, $module, $para['id'], $hid);
            }else{
                $query = "SELECT * FROM {$_M['table']['plist']} WHERE listid = '{$listid}' AND paraid='{$para['id']}' AND module={$module} AND lang = '{$_M['lang']}' ";
                $para_value = DB::get_one($query);
            }
            $paralist[$key]['value'] = $para_value['info'];
        }
        return $paralist;
     ...
     * @return mixed
     */
    public function delPlist($listid = '', $module = '')
    {
        global $_M;
