...
<form method="POST" action="{$url.own_name}c={$data.c}&a=dosaveinc&class1={$data.class1}&class2={$data.class2}&class3={$data.class3}&classnow={$data.list.classnow}" class='message-details-form' data-validate_order=".message-details-form" data-submit-ajax='1' enctype="multipart/form-data">
    <div div class="metadmin-fmbx">
        ...
        <dl>
            ...
            <dd class='form-group'>
                <input type="number" name="met_msg_time" value='{$data.list.met_msg_time}' min="0" class="form-control">
            </dd>
        </dl>
        <dl>
            <dt>
        ...
        <dl>
            ...
            <dd>
                <div class="custom-control custom-checkbox custom-control-inline">
                    ...
                    <label class="custom-control-label" for="met_msg_show_type1-{$checkbox_time}">{$word.messageincTip3}</label>
                </div>
            </dd>
        </dl>
        <h3 class='example-title border-none'>{$word.new_regist_admin_notice}</h3>
        <dl>
            <dt>
        ...
        <dl>
            ...
            <dd>
                <input type="text"  name="met_msg_to" value='{$data.list.met_msg_to}' data-plugin="tokenfield" data-delimiter="|" class="form-control">
        ...
        <dl>
            ...
            <dd>
                <input type="text"  name="met_msg_admin_tel" value='{$data.list.met_msg_admin_tel}' data-plugin="tokenfield" data-delimiter="|" class="form-control">
                <span class="text-help ml-2">{$word.fdincTip9}</span>
            </dd>
        </dl>
        <h3 class='example-title border-none'>
            {$word.feedbackauto}
            <a href="#/safe/?head_tab_active=1" target="_blank" class="btn btn-default ml-4 py-2"><i class="fa-envelope-o font-size-18 position-relative" style="top: 2px;"></i> {$word.mailboxSetTips}</a>
        </h3>
        <dl>
            <dt>
            ...
            <dd>
                <input type="checkbox" data-plugin="switchery" name="met_msg_back" value='{$data.list.met_msg_back}' data-checked='1' hidden>
                <span class="text-help ml-2">{$word.fdincTip10}</span>
            </dd>
        </dl>
        <dl>
            <dt>
        ...
        <?php
        $editor=array(
            'dt'=>$word['fdincAutoContent'],
            'no_title'=>1,
            'name'=>'met_msg_content',
            'height'=>300,
            'value'=>$data['list']['met_msg_content'],
            'tips'=>$word['fdinc_msg_content']
        );
        ?>
        <include file="pub/content_details/editor"/>
        <h3 class='example-title border-none'>
            {$word.feedbackautosms}
            <a href="#/app/met_sms" target="_blank" class="btn btn-default ml-4 py-2"><i class="fa-envelope-o font-size-18 position-relative" style="top: 2px;"></i> {$word.smsApiSetTips}</a>
        </h3>
        <dl>
            <dt>
            ...
            <dd>
                <input type="checkbox" data-plugin="switchery" name="met_msg_sms_back" value='{$data.list.met_msg_sms_back}' data-checked='1' hidden>
                <span class="text-help ml-2">{$word.fdincTipsms}</span>
            </dd>
        </dl>
        <dl>
            <dt>
                <label class='form-control-label'>{$word.phoneNumCheck}{$word.marks}</label>
            </dt>
            <dd>
                <input type="checkbox" data-plugin="switchery" name="met_msg_sms_check" value='{$data.list.met_msg_sms_check}' data-checked='1' hidden>
                <span class="text-help ml-2">{$word.phoneNumCheckTips}</span>
            </dd>
        </dl>
        <dl>
            <dt>
