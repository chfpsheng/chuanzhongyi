...
class column_handle extends handle
{
    ...
    public function para_handle($column)
    {
        ...
        foreach ($column as $key => $val) {
            $val['sub'] = 0;
            $return[$val['id']] = $this->one_para_handle($val,false);
        }

        //处理栏目是否有下级
        foreach ($return as $key => $val) {
            if ($val['bigclass'] && isset($return[$val['bigclass']])) {
     ...
     * @return array            处理过后的栏目数组
     */
    public function one_para_handle($content = array(),$text=true)
    {
        global $_M;
        $content['url'] = $this->get_content_url($content);
        $content['url_dynamic'] = $this->get_content_url($content, 1);
...
//        $type = $content['isshow'] == 0 ? 1: $type;
        ...
        $url_type = $this->url_type($type, $page_type);
...
{
    switch ($content['classtype']) {
        case 1:
            $classnum = 1;
            break;
        case 2:
            $classnum = $content['releclass'] ? 1 : 2;
            break;
        case 3:
            $bigclass = $this->cache_column[$content['bigclass']];
            $classnum = $bigclass['releclass'] ? 2 : 3;
            break;
        default:
            // 这里可以根据实际情况添加默认处理逻辑，暂时不做处理
            break;
    }
    return $classnum;
}
    ...
    public function url_dynamic($content)
    {
        ...
        if ($_M['config']['met_index_type'] == $content['lang'] && ($content['releclass'] || $classnum == 1)) {
            $url = $content['foldername'] . '/';
            return $this->url_transform($url, $content['lang']);
        }

        //url参数拼装
        $url_para = array();
        if (in_array($content['module'], array(2, 3, 4, 5, 6))) {
            $para_class = "class{$classnum}";
            $url_para[$para_class] = $content['id'];
        } elseif (in_array($content['module'], array(1, 8))) {
            $url_para['id'] = $content['id'];
        }

        if ($_M['config']['met_index_type'] != $content['lang']) {
            $url_para['lang'] = $content['lang'];
        }
        $query = http_build_query($url_para);

        $mod_name = $this->mod_to_name($content['module']);
        $index_name = $mod_name ? "{$mod_name}.php" : 'index.php';
        $url = "{$content['foldername']}/{$index_name}?{$query}";
    ...
    {
        global $_M;
        $class_level = $this->get_no_releclass($content);
        ...
        if ($content['filename']) {
            ...
            if (in_array($content['module'], array(7, 8))) {
                $url .= 'index';
            }
        }
        if (!$url && ($_M['config']['met_defult_lang'] || $_M['config']['met_index_type'] != $content['lang'])) {
            $url = 'index';
        }
        ...
        if ($_M['config']['met_defult_lang']) {
            ...
            if ($content['filename']) {//金太页面名称
                return $this->url_transform($content['foldername'] . '/' . $url . '.html', $content['lang']);
            }

            if ($class_level == 1) {//一级栏目无文件后缀
