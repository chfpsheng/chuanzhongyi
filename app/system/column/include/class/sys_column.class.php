...
class sys_column
{
    ...
    public function getColumnAction($column = array())
    {
        ...
        if ($column['classtype'] != 3 && in_array($column['module'], array(1, 2, 3, 4, 5 ,6))) {
            //可以添加下级栏目
...
//                            if ($sub2['module'] != $column['module'] && $sub2['module'] != 0 && in_array($sub2['module'], array(1, 2, 3, 4, 5, 6))) {
                            if ($sub2['module'] != $column['module'] && $sub2['module'] != 0) {
                                $diff_sub = 1;
                            }
                        }
    ...
    public function module($module)
    {
        global $_M;
        if ($module > 1000) {//应用模块
            $app = load::mod_class('myapp/myapp_op', 'new')->get_oneapp($module);
            if ($app) {
