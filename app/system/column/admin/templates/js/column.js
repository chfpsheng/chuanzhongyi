...
(function(){
    var that=$.extend(true,{}, admin_module),
            ...
            module_option:function(classtype,parent_module,fn){
                ...
                M.ajax({
                    ...
                    $.each(result, function(index, val) {
                        ...
                        //if(val.mod==6 && val.num && parent_module!=6)) return true;
        ...
        columnList=function(data){// 栏目列表数据渲染
            ...
            $.each(data, function(index, val) {
                ...
                if(val.subcolumn){
                    name_html=`<div class="position-relative">
                            ${name_html}
                            <a href="javascript:;" class="px-1 text-dark noshow position-absolute btn-show-subcolumn"><i class="fa-angle-down"></i></a>
                        </div>`;
                }
                ...
                var is_list_module=[2,3,4,5].includes(val.module);
                if(!is_list_module && val.subcolumn){
                    var handle=(list)=>{
                            $.each(list, function(index, value) {
                                if(!is_list_module && [2,3,4,5].includes(parseInt(value.module))) is_list_module=1;
                                if(!is_list_module && value.subcolumn) handle(value.subcolumn);
                            });
                        };
                    handle(val.subcolumn);
                }
                list.push(`<button type="button" class="btn btn-light-primary mr-1" data-toggle="modal" data-target=".column-details-modal" data-modal-title="${METLANG.columnmeditor}" data-modal-size="lg" data-modal-url="column/edit/${edit_dataurl+val.id}" data-modal-fullheight="1"  data-modal-height100="1" data-modal-tablerefresh="${column_list}">${METLANG.seting}</button>
                <div class="dropdown d-inline-block">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        ${action_more}
                        ${M.component.btn('del',{del_url:delurl+val.id,class:'dropdown-item btn-del-column'+(is_list_module?' list-module':''),confirm_title:METLANG.delete_information+(is_list_module?METLANG.jsx39:'')})}
                    </div>
                </div>`);
                ...
                if(val.classtype>1) list['DT_RowClass']+=' bg-grey-100 hidden';
        ...
        return {
            ajax:{
                dataSrc:function(result){
                    ...
                    column_list_data=result.data.column_list;
    ...
    // 删除栏目确认弹框
    $column_list.find('[table-delete]').addClass('list-module btn-del-column');
    $column_list.on('click','tfoot .btn-del-column',function(){
        setTimeout(()=>{
            const allid=$column_list.parents('form').find('[name="allid"]').val()||'';
            $(this).attr('data-url',$column_list.parents('form').attr('action')+`&submit_type=del&allid=${allid}`);
        },0);
    });
    $column_list.on('click','.btn-del-column',function(){
        $(this).hasClass('list-module') && setTimeout(()=>{
            $('.alertify .dialog.confirm>div').width(600).find('nav .ok').html('删除栏目（彻底删除内容）').removeClass('btn-primary').addClass('btn-light-danger border-trans').after(`<button class="ok btn btn-primary btn-del-column-recycle" data-url="${$(this).attr('data-url')}&recycle=1">删除栏目（将内容放入回收站）</button>`);
            setTimeout(() => {
    ...
    // 删除栏目（内容列表放入回收站）
    $(document).on('click','.alertify .btn-del-column-recycle',function(){
        var url=$(this).attr('data-url');
        $column_list.parents('form').removeAttr('data-submited');
        M.ajax({
            url,
            success:res=>{
                delColumnCallback(url);
                $column_list.tabelAjaxFun(res);
            }
        });
        $('.alertify').remove();
    });
    // 删除栏目后处理可视化窗口跳转
    function delColumnCallback(url){
        if(!M.is_admin){
            var id=url.match(/id=([^&]*)(&|$)/)[1].split(','),
                page_iframe_window=$('.page-iframe').prop('contentWindow'),
                handle=((list,parent_has_del=0)=>{
                    list.forEach((value, index, array) => {
                        if(parent_has_del) id.push(value.id);
                        value.subcolumn && value.subcolumn.length && handle(value.subcolumn,id.includes(value.id)||parent_has_del);
                    });
                });
            handle(column_list_data);
            if(id.includes(page_iframe_window.M.classnow).toString()) setCookie('page_iframe_url','');
        }
    }
    // 列表渲染后回调
    ...
    M.load('form',function(){
        formSaveCallback($column_list.parents('form').attr('data-validate_order'),{
            true_fun:function(res){
                res.url && res.url.indexOf('submit_type=del')>0 && delColumnCallback(res.url);
    ...
    $column_list.find('.btn-show-allsubcolumn').click(function(event) {
        ...
        var is_down=$('i.fa-rotate-180',this).length;
        obj.find('.btn-show-allsubcolumn2 span').html(METLANG[(is_down?'open':'close')+'_allchildcolumn_v6']);
        obj.find('.btn-show-allsubcolumn2 i:not(.transition500)').addClass('transition500');
        obj.find('.btn-show-allsubcolumn2 i')[`${is_down?'remove':'add'}Class`]('fa-rotate-180');
        if(is_down){
            ...
            $column_list.find('tbody tr').removeClass('hide hidden').find('td>div').stop().slideDown(300).find('.btn-show-subcolumn:not(.noshow) i').addClass('fa-rotate-180');
            if(!allsubcolumn){
                allsubcolumn=1;
            ...
            toggleColumn=function(objs,hide){
                ...
                if($icon.hasClass('fa-rotate-180')||!is_click){
                    ...
                    if(is_click){
                        ...
                        if(bigclass && !$bigclass_btn_show_subcolumn.find('i').hasClass('fa-rotate-180')) $bigclass_btn_show_subcolumn.click();
                        setTimeout(()=>{
    ...
    M.load('modal',function(){
        M.component.modal_options[column_add_modal]={
            ...
            modalDialogStyle:'margin-top:10%;',
            modalRefresh:'one',
            modalBody:`<form action="${that.own_name}c=index&a=doAddColumn" class="form-inline" data-submit-ajax="1">
                <table class="table table-hover w-100 dtr-inline">
                    ...
                    <tfoot>
                        <tr>
                            <th colspan="6">
                                <button type="button" class="btn btn-outline-666 btn-add-columns">${METLANG.add}</button>
                            </th>
                        </tr>
                    </tfoot>
                </table>
    ...
    function sidebarReload(){
        if($metadmin_sidebar_nav.length){
            var active_key=$metadmin_sidebar_nav.find('li.active').index('.metadmin-sidebar-nav li');
            getAdminMenus(
                {
                    left_nav:1,
                },
