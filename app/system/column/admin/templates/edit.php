...
<form method="POST" action="{$url.own_name}c=index&a=doEditorsave" class='column-details-form' data-validate_order=".column-details-form" data-submit-ajax='1' enctype="multipart/form-data">
	...
	<div class="metadmin-fmbx">
		...
		<dl>
			...
			<dd>
				...
				<div class="border p-2 rounded-lg">
					<div class="clearfix">
						<span class="text-help text-content float-left mr-1">{$word.text_size}{$word.marks}</span>
						...
						<span class="text-help text-content float-left mr-1">{$word.text_color}{$word.marks}</span>
						<input type="text" name="text_color" value="{$data.list.text_color}" data-plugin="minicolors" class="form-control w-a d-inline-block">
					</div>
					<div class="text-help border-top d-block">{$word.column_style_tips}</div>
				</div>
			</dd>
		</dl>
		<dl>
        ...
        <dl>
            <dt>
                <label class='form-control-label'>{$word.columnstyle}</label>
            </dt>
            <dd>
                <div class='form-group clearfix'>
                    <select class="form-control w-a" name='style_type' data-checked="{$data.list.style_type}">
                        <list data="$data['style_type_list']" name="$v">
