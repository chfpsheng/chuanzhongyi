...
//            @unlink(PATH_WEB . 'web.config');
    ...
    protected function createApacheHttpdurl($metbase = '')
    {
        ...
        $htaccess .= 'RewriteRule ^(.*)\.(asp|aspx|asa|asax|dll|jsp|cgi|fcgi|pl)(.*)$ 404.html [L]' . "\n";
        $htaccess .= 'RewriteRule index\.php/\w+ 404.html [L]' . "\n";
        $htaccess .= 'RewriteCond %{REQUEST_FILENAME} -f' . "\n";
        $htaccess .= 'RewriteRule (.+) $1 [L,QSA]' . "\n";
    ...
    protected function createIISHttpdurl($metbase = '')
    {
        ...
        $web .= '<match url="^/public/third-party/ueditor/([a-zA-Z0-9_^\x00-\xff]+).html$" />' . "\n";
        $web .= '<action type="Rewrite" url="/public/third-party/ueditor/$1.html" />' . "\n";
        $web .= '</rule>' . "\n";
