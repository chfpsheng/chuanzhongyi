...
class relation_handle extends base_handle
{
    ...
    public function para_handle($list = array(), $para = 0)
    {
        ...
        foreach ($list as $key => $row) {
            ...
            $relation_class = $one['relation_class'];
            if (!in_array($relation_class, $arr)) {
                $class_list[$relation_class]['classnow'] = $one['relation_class'];
                $class_list[$relation_class]['relation_module'] = $one['relation_module'];
                $class_list[$relation_class]['relation_class_name'] = $one['relation_class_name'];
            }
            $class_list[$relation_class]['list'][] = $one['content'];
        }

        return array_values($class_list);
    }
    ...
    public function one_para_handle($data = array())
    {
        ...
        $content['para'] = load::mod_class('parameter/parameter_label', 'new')->get_parameter_contents($data['relation_module'], $data['relation_id'], $content['class1'], $content['class2'], $content['class3']);
