 ...
 */
 (function(){
					...
					if(result.data){
						$.each(result.data, function(index, val) {
							var item=[
									...
									listStatusHtml(val.id,'state'),
									listStatusHtml(val.id,'op'),
                                ];
							data.push(item);
						});
					}
	...
	// 取消关联
	$(document).on('click','tfoot .btn-light,tbody .relation-del','#content-relation-list', function () {
		M.load('alertify',()=>{
		...
		$checked.each(function(){
			...
			if(type){
				if(!relation_list[info]){
					...
					$tr.find('td:eq(2)').html(listStatusHtml(id,'state'));
					$tr.find('td:eq(3)').html(listStatusHtml(id,'op'));
				}
			}else{
				if(relation_list[info]){
	...
	// 关联内容状态html
	function listStatusHtml(id,type){
		var is_in=relation_list[id+'|'+class_info.module];
		if(type=='state'){
			return is_in?`<span class="badge font-weight-normal py-2 px-4 badge-pill badge-light-success">${METLANG.relation_checked}</span>`:``;
		}
		return is_in?`<button type="button" class="btn btn-light relation-del">${METLANG.relation_cancel}</button>`:`<button type="button" class="btn btn-outline-666 relation-add">${METLANG.relation_add}</button>`;
	}
})();