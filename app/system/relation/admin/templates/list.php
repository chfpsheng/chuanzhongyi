...
<div class="content-relation-body">
    ...
    <form action="" method="post" data-content_info="{$data.module}|{$data.content_id}">
        ...
        <table class="table table-hover dataTable w-100 mt-2" id="content-relation-list" data-ajaxurl="{$url.adminurl}n=relation&c=relation_admin&a=doGetDatelist" data-plugin="checkAll" data-datatable_order="#content-relation-list">
            <thead>
                <tr>
                    <include file="pub/content_list/checkall_all"/>
                    <th>{$word.title}</th>
                    <th data-table-columnclass="text-center">{$word.state}</th>
                    <th>{$word.operate}</th>
                </tr>
            </thead>
            <tbody>
            ...
            <include file="pub/content_list/tfoot_first"/>
                    <button type="button" class="btn btn-light mr-1">{$word.relation_cancel}</button>
                    <button type="submit" class='btn btn-outline-666 px-4'>{$submit_text}</button>
                </th>
            </tfoot>
        </table>
    </form>
