...
class index extends base_admin
{
    ...
    public function doGetContentList()
    {
        ...
        $redata = array();
        $class = $_M['form']['class'];
        $content_type = $_M['form']['content_type'] ?: 0;
        $contents = $content_type ? self::getContentBymodule() : self::getContentByColumn();
        $redata['status'] = 1;
        $redata['data']['list'] = $contents;
            ...
            if ($mod > 0 && $mod <= 8) {
                ...
                if ($mod == 6) {
                    ...
                    foreach ($column as $row) {
                        $class123 = $_M['class']['column_label']->get_class123_no_reclass($row['id']);
                        $job_column = array();
    ...
    private function _search($search_contnet = '')
    {
        ...
        foreach ($column_list as $key1 => $class1) {
            ...
            $class1_son = $column->get_column_son($class1['id']);
            if ($class1_son) {
                foreach ($class1_son as $key2 => $class2) {
                    $class2['url'] = self::getModContentLink($class2);
                    $class2_son = $column->get_column_son($class2['id']);
                    if ($class2_son) {
    ...
    private function getColumnjson($module = '', $tolang = '')
    {
        ...
        foreach ($array['class1'] as $key => $val) { //一级级栏目
            ...
            if ($val['module'] == $module) {
                ...
                if (is_array($array['class2'][$val['id']]) && count($array['class2'][$val['id']])) { //二级栏目
                    $k = 0;
                    ...
                    foreach ($array['class2'][$val['id']] as $key => $val2) {
                        ...
                        if (is_array($array['class3'][$val2['id']]) && count($array['class3'][$val2['id']])) { //三级栏目
                            $j = 0;
            ...
            foreach ($val as $val2) {
                if ($val2['module'] == $module) {
                    if ($val2['releclass']) {
                        ...
                        if (is_array($array['class3'][$val2['id']]) && count($array['class3'][$val2['id']])) {
