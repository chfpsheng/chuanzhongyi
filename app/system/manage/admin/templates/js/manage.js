...
(function(){
	...
	var that=$.extend(true,{},admin_module),
		...
		columnList=function(data,sub,view_type){
			...
			$.each(data, function(index, val) {
				...
				html+='<'+tagname+' class="'+(sub==1?'py-1 pl-1 d-block':'font-size-12 mt-1')+'">'
					+'<a href="'+(!M.is_admin&&info.substr(0,2)=='#/'?info:'javascript:;')+'" '+(info?'data-info="'+info+'"':'')+' data-classtype="'+val.classtype+'" class="list-group-item border-none d-flex align-items-center justify-content-between rounded-10 pr-2 pl-'+(1+sub)+'">'
						+'<span class="'+(sub==3?'pl-2':'')+' pr-1">'+val.name+'</span>'
						+(val.classtype!=3?'<span class="btn-column-slide '+(val.subcolumn?'':'invisible')+'"><i class="fa-angle-right font-size-16 transition500"></i></span>':'')
					+'</a>';
			...
			if(sub == 1){
				$default_show_column.html(view_type ? '' : default_show_column_html).find('option[value="' + (getCookie('manage_default_show_column') || '') + '"]').attr({'checked':''});
				obj.find('.default_show_column-wrapper')[`${view_type?'add':'remove'}Class`]('hide');
			}
	...
	var columnView=function(view_type){
			var $btn_pageset_nav_manage=$('.pageset-head-nav [data-url="manage"]'),
				...
				view_type=location_view_type||view_type||$content_btn_group.find('button.active').data('view_type'),
			...
			$list.html(`<div class="d-flex align-items-center justify-content-center list-loader"><div class="loader loader-round-circle"></div></div>`);
			...
			$.ajax({
				...
				success:function(result){
					...
					M.hasGtContentList=1;
				}
			});
			if(location_view_type){
				$content_btn_group.find('button[data-view_type="'+location_view_type+'"]:not(.active)').addClass('active').siblings().removeClass('active');
				$('#content-view-'+location_view_type+':not(.active)').addClass('active show').siblings().removeClass('active show');
			}
		};
	columnView();
	var maxh=$(window).height()-($('.metadmin-head').height()||0)-($('.metadmin-head').length?300:353);
	...
	// 列表显示类型切换
	$content_btn_group.find('button').click(function(event) {
	...
	obj.on('click', '.column-view-list .list-group-item .btn-column-slide', function(event) {
		$('i',this).toggleClass('fa-rotate-90').parents('.list-group-item').next('.subcolumn').slideToggle();
	...
	function contentShow(options){
		var $loader=$content_show.find('.met-loader'),
			...
			filename='',
			is_listmodule=$.inArray(options.module, ['news','product','img','download','activity','doctor','shaoer'])>=0?1:0,
			loadFun=function(){
				if(is_listmodule || options.module=='job'){
					M.loadedList=0;
					if(options.module=='product' && parseInt($content_show.data('shopv2_goods_set')||0)) M.loadedList=1;
					interval({
						true_val:(time)=>{
							return M.loadedList;
						},true_fun:(time)=>{
				...
				setTimeout(()=>{
					var $class1=admin_module.obj.find('[data-plugin="select-linkage"] [name="class1"]');
					if(!$class1.length) return;
					if(view_type=='column'){
						$class1.attr('disabled',true).addClass('disabled');
					}else{
						$class1.attr('disabled',false).removeClass('disabled');
					}
				},500);
			};
		if(is_listmodule){
		...
		if($content_show.find('.content-show-item[data-path="'+hash+'"]').length && $content_show_item.attr('data-loaded')){
			...
			metLoadTemp(url,options,$content_show_item,function(html){
				if(!is_listmodule) $loader.addClass('hide');
				$content_show.append(`<div class="content-show-item" ${is_listmodule?`style="display:none;"`:''} data-path="${hash}" data-loaded="1"></div>`);
                    ...
                    setTimeout(function(){
                        formSaveCallback($content_show_item.find('form').attr('data-validate_order'),{
                            true_fun: function(result) {
								...
								result.data && result.data.html_res && contentCreateHtml(result.data.html_res);
                            }
                        });
                    },500);
                });
	...
	obj.find('.btn-column-control').click(function(event) {
		...
		$('i',this).attr({class:'fa-angle-double-'+($column_view_wrapper.hasClass('min')?'right':'left')});
	});
	// 搜索栏目
