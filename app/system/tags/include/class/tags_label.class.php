...
class tags_label extends base_label
{
    ...
    public function get_tags_list($data = array(), $lang = '')
    {
        ...
        $tags = DB::get_all($query);
        foreach ($tags as &$val) {
            $url = $this->getTagUrl($val);
            if (!$url) {
                continue;
            }
            $val['url'] = $url;
            $val['tag_name'] = htmlspecialchars($val['tag_name'], ENT_QUOTES, 'UTF-8');
            $val['title'] = htmlspecialchars($val['title'], ENT_QUOTES, 'UTF-8');
            $val['keywords'] = htmlspecialchars($val['keywords'], ENT_QUOTES, 'UTF-8');
            $val['description'] = htmlspecialchars($val['description'], ENT_QUOTES, 'UTF-8');
            $style = '';
            if ($val['tag_size']) {
    ...
    public function getSqlByTag($tag_name, $class)
    {
        ...
        $listid = array();
        foreach ($tag as $val) {
            if (empty($val['list_id'])) continue;
            foreach (explode('|', trim($val['list_id'], '|')) as $id) {
                if (empty($val['list_id'])) continue;
                foreach (explode('|', trim($val['list_id'], '|')) as $id) {
                    // 先对 $id 进行去除首尾空白字符操作，再判断是否不为空
                    $trimmedId = trim($id);
                    if (!empty($trimmedId)) {
                        $listid[] = $trimmedId;
                    }
    ...
    public function getTagUrl($tag, $type = null)
    {
        ...
        if ($_M['config']['tag_search_type'] == 'module') {
            ...
            //语言栏目
            $_M['class']['column_label']->get_column($lang);
            $column = $_M['class']['column_label']->get_column_id($cid);
            if (!$column) {
                return false;
            }
