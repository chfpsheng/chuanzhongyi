        ...
        <div class="row">
            <div class="col-lg-12 met-cons">
                <div class="met-news-list news-lists" m-id="noset">
                    <ul class="met-page-ajax met-pager-ajax" data-scale='<?php echo $c['met_doctorimg_y'];?>x<?php echo $c['met_doctorimg_x'];?>'>
                        <?php
    $cid = 0;
    if($cid == 0){
        $cid = $data['classnow'];
    }
    $num = $c['met_doctor_list'];
    $order = "no_order";
    $doctor = load::sys_class('label', 'new')->get('doctor');
    $doctor->page_num = $num;
    $result = $doctor->get_list_page($cid, $data['page']);
    $sub = is_array($result) ? count($result) : 0;
     foreach($result as $index=>$v):
        $v['sub']      = $sub;
        $v['_index']   = $index;
...
?>
                        <li>
                            ...
                            <a class="img" href="<?php echo $v['url'];?>" title="<?php echo $v['title'];?>" <?php echo $g['urlnew'];?>>
                                <img src="<?php echo thumb($v['imgurl'],$c['met_doctorimg_x'],$c['met_doctorimg_y']);?>" alt="<?php echo $v['title'];?>">
                            </a>
                            <?php } ?>
                            <h4>
