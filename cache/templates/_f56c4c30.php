...
<?php } ?>
  ...
  <?php if($ui['ui_show']){ ?>
  <?php if($data['classnow']<>10001){ ?>
<section class="location_met_m1156_7_16_main met-content animsition"
  ...
  <div class="container">
    <div class="row">
      <div class="  <?php if($ui["has"]["sidebar"]){ ?>col-lg-9<?php }else{ ?>col-lg-12<?php } ?> met-cons">
        <div class="location_met_m1156_7_16 met-position  pattern-show" m-id="<?php echo $ui['mid'];?>" m-type="nocontent">
          <div class="container">
            <div class="row">
...
<section class="location_met_m1156_7_16" m-id="<?php echo $ui['mid'];?>" m-type="nocontent" 
...
<div class="para_search_met_16_1_58" m-id='<?php echo $ui['mid'];?>'>
...
<?php } ?>
        ...
        <?php
            $id = 22;
            $panrent_name = 'product_list_page';
            $style = 'met_m1156_7';
            $skin_name = 'm1156ui010';
            if(!isset($parse)){
                include_once PATH_ALL_APP . 'met_template/include/class/parse.class.php';
...
<section class="product_list_page_met_m1156_7_22_main met-content animsition" 
  data-background="  <?php if($ui["bgok"]){ ?><?php echo $ui['bgimg'];?><?php } ?>">
  <div class="container">
    <div class="row">
      <div class="  <?php if($ui["has"]["sidebar"]){ ?>col-lg-9<?php }else{ ?>col-lg-12<?php } ?> met-cons">
<?php } ?> 
        ...
        <div class="product_list_page_met_m1156_7_22" m-id="<?php echo $ui['mid'];?>">
          <div class="met-img">
...
?>
              <li class="parent-slide shown page1">
                <a href="<?php echo $m['url'];?>" title="<?php echo $m['name'];?>" <?php echo $m['urlnew'];?>>
                  <span><img data-original="<?php echo thumb($m['columnimg'],$c['met_productimg_x'],$c['met_productimg_y']);?>" alt="<?php echo $m['name'];?>"></span>
                    <?php if(($ui['listtype']==1 && $data['index_num']<>2 && $data['index_num']<>3) || $data['index_num']==1){ ?>
                  <h4 class="on"><?php echo $m['name'];?></h4>
                  <?php }else if(($ui['listtype']==2 && $data['index_num']<>1 && $data['index_num']<>3) || $data['index_num']==2){ ?>
                  <h5><em><?php echo $m['name'];?></em></h5>                  
                  <?php }else if(($ui['listtype']==3 && $data['index_num']<>2 && $data['index_num']<>1) || $data['index_num']==3){ ?>
                  <h4><?php echo $m['name'];?></h4> 
                  <b><?php echo $m['namemark'];?></b>
    ...
    $para = 0;
    $order = "no_order";
    $result = load::sys_class('label', 'new')->get('product')->get_list_page($cid, $data['page'] ,$para);
    $sub = is_array($result) ? count($result) : 0;
     foreach($result as $index=>$v):
        $v['sub']      = $sub;
        $v['_index']   = $index;
...
?>
              <li class="parent-slide shown page1">
                <a href="<?php echo $v['url'];?>" title="<?php echo $v['title'];?>" <?php echo $g['urlnew'];?>>
                  <span><img data-original="<?php echo thumb($v['imgurl'],$c['met_productimg_x'],$c['met_productimg_y']);?>" alt="<?php echo $v['title'];?>"></span>
                    <?php if(($ui['listtype']==1 && $data['index_num']<>2 && $data['index_num']<>3) || $data['index_num']==1){ ?>
                  <h4 class="on">
                    <?php echo $v['title'];?>
                      <?php if($v['price_str']){ ?>
                    <strong class="price"><?php echo $v['price_str'];?></strong>
                    <?php } ?>
                  </h4>
                  <?php }else if(($ui['listtype']==2 && $data['index_num']<>1 && $data['index_num']<>3) || $data['index_num']==2){ ?>
                  <h5>
                  ...
                  <b>                
                            <?php
            $sub = is_array($v['para']) ? count($v['para']) : 0;
            $cycleindex = 50;

            if(!is_array($v['para']) && $v['para']){
            ...
            foreach ($v['para'] as $index => $val) {
                ...
                $t = $val;
            ?> 
                    <font><?php echo $t['name'];?> : <?php echo $t['value'];?></font>
                    <?php }?>
                  </b>
                  <p><?php echo str_replace("\n",'<br>',$v['description']);?></p>
                  <dl>
                    <dt><?php echo $word['Detail'];?></dt>
                    <dd><i class="icon wb-eye" aria-hidden="true"></i> <?php echo $v['hits'];?></dd>
                  </dl>
                  <?php } ?>
                </a>
              </li>
...
<?php } ?>
        ...
        ?> 
  <?php if($ui['ui_show']){ ?>
      <div class="sidebar_met_m1156_7_13 col-lg-3 met-conx" m-id="<?php echo $ui['mid'];?>" m-type="nocontent">
...
<?php }else if($_GET['pageset']){ ?>
<section class="sidebar_met_m1156_7_13" m-id="<?php echo $ui['mid'];?>" m-type="nocontent" 
  style="background:#263238;max-height:40px;text-align:center;color:#fff;line-height:40px;display:block;border-bottom:1px solid #888;">
  该栏目设置了侧边栏隐藏，可在“侧边栏配置”中设置显示（该文字仅“可视化”模式下可见）
</section>
<?php } ?>
