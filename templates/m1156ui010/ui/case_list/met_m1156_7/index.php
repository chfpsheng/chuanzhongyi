<?php defined('IN_MET') or exit('No permission'); ?>
<tag action="category" type="current" cid="$data['classnow']"></tag>
<if value="$data['classnow'] eq 10001||!$ui['bgcolumn']||strstr('|'.strip_tags($ui['bgcolumn']).'|','|'.strip_tags($m['name']).'|')">
<section class="$uicss lazy {$ui.bgfull}" m-id="{$ui.mid}" data-title="{$ui.bgtitle}" 
  data-background="<if value='$ui["bgok"]'>{$ui.bgimg}</if>">
  <div class="container">
    <div class="title-box"><h2>{$ui.title}</h2><p>{$ui.description}</p></div> 
    <div class="row">
	  <div class="parent-box swiper-container-horizontal">
        <ul class="parent-wraper">
          <tag action="list" cid="$ui['columnid']" type="$ui['type']" num="$ui['number']">  
		  <li class="parent-slide">
			<a href="{$v.url}" title="{$v.title}" {$g.urlnew}>
			  <span><img class="parent-lazy" src="{$c.met_agents_img|thumb:$ui['width'],$ui['height']}" data-src="{$v.imgurl|thumb:$ui['width'],$ui['height']}" alt="{$v.title}"></span>
			  <h4>{$v.title}</h4>
              <if value="$ui['tagok']&&$v['tag']">
			  <b>
                <list data="$v['tag']" name="$t"> 
                <i class="fa fa-check-square-o"></i>
                {$t} &nbsp;
                </list>
              </b>
              </if>
			  <p><if value="$v['description']">{$v.description|str_replace:"\n",'<br>',@@}</if></p>
			</a>
		  </li> 
          </tag> 
	    </ul>
	  </div>
    </div> 
    <tag action="category" cid="$ui['columnid']" type="current">
    <div class="about-link text-center">
      <a href="{$m.url}" title="{$m.name}" {$m.urlnew}>
        <span>{$ui.more}</span>
        <i class="fa fa-angle-double-right"></i>
      </a> 
	</div>
    </tag>  
  </div> 
</section>
<elseif value="$_GET['pageset']" />
<section class="$uicss" m-id="{$ui.mid}" m-type="nocontent" 
  style="background:#263238;max-height:40px;text-align:center;color:#fff;line-height:40px;display:block;border-bottom:1px solid #888;">
  该栏目设置了限制显示，可在“区块显示的栏目”中添加显示（该文字仅“可视化”模式下可见）
</section>
</if>