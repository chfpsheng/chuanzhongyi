...
<section class="$uicss <if value='$ui["heightfull"]&&($ui["heightfull"]!=2||$data["classnow"]==10001)'>full</if>" 
  ...
  <list data="$array" name="$ui_mod">

  <if value="strip_tags($ui_mod) eq '简介模块'&&$data['module']==1">hidden</if>

  <if value="strip_tags($ui_mod) eq '新闻模块'&&$data['module']==2">hidden</if>

  <if value="strip_tags($ui_mod) eq '产品模块'&&$data['module']==3">hidden</if>

  <if value="strip_tags($ui_mod) eq '下载模块'&&$data['module']==4">hidden</if>

  <if value="strip_tags($ui_mod) eq '图片模块'&&$data['module']==5">hidden</if>

  <if value="strip_tags($ui_mod) eq '招聘模块'&&$data['module']==6">hidden</if>

  <if value="strip_tags($ui_mod) eq '留言模块'&&$data['module']==7">hidden</if>

  <if value="strip_tags($ui_mod) eq '反馈模块'&&$data['module']==8">hidden</if>

  <if value="strip_tags($ui_mod) eq '全站搜索'&&$data['module']==11">hidden</if>

  <if value="strip_tags($ui_mod) eq '网站地图'&&$data['module']==12">hidden</if>
