 ...
 (function(){
    ...
    M.component.modalFun=function(options){
        ...
        var html=`<div class="modal fade met-scrollbar met-modal ${options.modalOtherclass} ${options.modal_class_name}" id="${options.id}" data-key="${options.modal_class}" data-keyboard="${options.modalKeyboard}" data-backdrop="${options.modalBackdrop}" ${options.modalSubmitNoclose?'data-submit-noclose="1"':''} style="${options.modalStyle}">
                <div class="modal-dialog modal-dialog-${options.modalType} modal-${options.modalSize} ${options.modalFullheight?'modal-dialog-scrollable':''} ${options.modalHeight100?'h-100':''}" style="${options.modalDialogStyle}">
                    <div class="modal-content ${options.modalContentclass}">
                        <div class="modal-header d-block bg-grey-100 border-none clearfix ${options.modalHeaderclass}">
                            <h6 class="modal-title float-left font-weight-bold ${options.modalTitleclass}">${options.modalTitle}</h6>
                            ${options.modalHeadercenter}
                            ${options.modalClose?'<button type="button" class="close h2" data-dismiss="modal" aria-label="Close"><span>×</span></button>':''}
                        ...
                        ${options.modalFooterok?`<div class="modal-footer clearfix d-block bg-grey-100 border-none ${options.modalFooterclass}">
    ...
    $(document).on('show.bs.modal shows', '.modal', function(event) {
        ...
        setTimeout(function(){
            var options=M.component.modal_options[key]||{},
                url=$modal_body.attr('data-url'),
                ...
                callback=function(data){
                    ...
                    setTimeout(function(){
                        var loadFun=function(){
                                ...
                                typeof options.callback=='function'&&options.callback(key,data,refresh);
                            };
                ...
                loadTemp=function(data){
                    ...
                    if(url && refresh!='0'){
                        ...
                        metLoadTemp(url,data,$modal_body.find('.modal-html'),function(html){
                            ...
                            key && admin_module.obj.parents(key).length && _TEMPLOADFUN(admin_module.hash);
                        ...
                        key && key!='.pageset-nav-modal' && typeof admin_module!='undefined' && admin_module.obj.parents(key).length && _TEMPLOADFUN(admin_module.hash,1);
                        $modal_body.scrollTop(0);
                        callback(data);
                    }
            ...
            typeof options.before=='function'&&options.before(key);
            if(url && $modal_body.attr('data-dataurl')){
                ...
                if(dataurl.indexOf('M.list')>=0){
                    ...
                    if(url && refresh!='0') $modal_body.find('.modal-loader').removeClass('hide');
                    M.ajax({
                        url: dataurl,
