 ...
 */
(function(){
    var datatable_langurl= M.url.public_third+'datatables/language/';
    // datatable多语言选择
    ...
    function deleteLocalStorage(table_id){
        // 如果是重新进入页面，则清除DataTable表格的Local Storage，清除本插件stateSave参数保存的表格信息
        if(!performance.navigation.type || M.url.admin){
            for(var i=0;i<localStorage.length;i++){
                if(localStorage.key(i).indexOf('DataTables_'+(table_id?`${table_id}_${location.pathname}`:''))>=0) localStorage.removeItem(localStorage.key(i));
            }
        }
    }
    deleteLocalStorage();
    ...
    window.datatableOption=function(obj,datatable_order){
        ...
        var datatable_order=datatable_order||0,
            ...
            option={
                ...
                initComplete: function(settings, json) {// 表格初始化回调函数
                    ...
                    $wrapper.find('thead th,tfoot th,.dataTables_footer').addClass('position-sticky');
                    $wrapper.find('thead th').addClass('bg-grey-100');
                    $wrapper.find('tfoot th,.dataTables_footer').addClass('bg-white');
                    ...
                    if($(this).width()>$wrapper.width()) $wrapper.addClass('oxa met-scrollbar');
                },
                drawCallback: function(settings){// 表格重绘后回调函数
                    ...
                    $(this).parents('.dataTables_wrapper').metTableLoader('hide');
                    M.loadedList=1;
                    // 功能渲染
        ...
        if(typeof new_option!='undefined'){
            if(new_option.columnDefs) delete new_option.columnDefs;
            if(new_option.ordering){
                var targets=[];
                obj.find('thead th:not([data-order_info])').each(function(i){
                    targets.push($(this).index());
                });
        ...
        obj.on('preXhr.dt', function (e, settings, data) {
            $(this).parents('.dataTables_wrapper').metTableLoader();
        });
        return option;
    };
    $.fn.extend({
        ...
        metDataTable:function(fn){
            ...
            M.load('datatables',function(){
                $dataTable.each(function(index, el) {
                    var order=$(this).attr('data-datatable_order')?$(this).attr('data-datatable_order'):`#${$(this).attr('id')?$(this).attr('id'):('met-datatable-'+new Date().getTime()+index)}`;
                    $(this).attr({'data-datatable_order':order});
                    !$(this).attr('id') && $(this).attr({id:order.substr(0,1)=='#'?order.substr(1):order});
                    deleteLocalStorage($(this).attr('id'));
        ...
        metFormAjaxDel:function(){
            ...
            if(url?($(this).parents('tbody').length?1:allid!=''):allid!=''){
                ...
                var fn_name=$(this).attr('data-fn');
                if(fn_name && typeof eval(fn_name)=='function') data=eval(`${fn_name}(data,url,$(this),$table)`);
                M.ajax({
                    url: url||$form.attr('action'),
                    data:data,
                    success: function(result){
                        result.url=url||$form.attr('action');
                        result.ajax_data=data;
                        $table.tabelAjaxFun(result);
                    }
                });
            }else{
                M.load('alertify',function(){
                    alertify.alert(METLANG.jslang3||'请选择至少一项','','','tips');
                });
            }
        },
        // 加载进度过渡动画
        metTableLoader:function(handle,delay,bg='white'){
            handle=handle||'show';
            delay=typeof delay!='undefined'?delay:300;
            var $loader=$('>.table-loader',this);
            if(!$loader.length){
                $(this).each(function(index,val){
                    if($(this).css('position')=='static') $(this).addClass('position-relative');
                    // $(this).append(`<div class="table-loader h-100 w-100 position-absolute bg-${bg} hide" style="left:0;top:0;"><div class="text-center h-100 w-100 d-flex align-items-center"><div class="loader loader-round-circle"></div></div></div>`);
                    $(this).append(`<div class="table-loader h-100 w-100 position-absolute bg-${bg} hide" style="left:0;top:0;"><div class="text-center h-100 w-100 d-flex align-items-center justify-content-center text-secondary my-0">${METLANG.fliptext2}...</div></div>`);
                });
                $loader=$('>.table-loader',this);
            }
            if($loader.hasClass('hide')) $loader.removeClass('hide');
            if(handle=='show'){
                $(this).css({'min-height':$(this).outerHeight()<90?90:''});
                $loader.show();
            }else if(handle=='hide'){
                $(this).css({'min-height':''});
    ...
    $(document).on('change',"[data-table-search]",function(){
        ...
        if(typeof datatable[datatable_order]!='undefined'){
            if($(this).attr('type')=='search'){
                $(this).attr('data-searching',1);
                setTimeout(()=>{
                    $(this).removeAttr('data-searching');
                },200);
            }
            datatable[datatable_order].ajax.reload();
        }
    });
    // 自定义搜索框-点击搜索按钮
    $(document).on('click','input[type="search"][data-table-search]+.input-group-append',function(){
        if(!$('.fa-search',this).length || $(this).parents('.form-group').hasClass('has-danger') || M.dataTable_search_stop) return false;
        var $search=$(this).parents('.input-group').find('[data-table-search]');
        if($search.attr('data-searching') || !$search.val()) return;
        var $this_datatable=$(this).parents('.dataTable').length?$(this).parents('.dataTable'):$($search.data('table-search')),
        datatable_order=$this_datatable.attr('data-datatable_order');
