...
(function(){
    $(function(){
        ...
        $(document).on('change', 'input[type="radio"][data-target]', function(event) {
            ...
            ($target.removeClass('hide'),$(this).parents('form').find('input[type="radio"][data-target][name="'+$(this).attr('name')+'"][value!="'+$(this).val()+'"]').each(function(index, el) {
            ...
            $(this).each(function(index, el) {
                ...
                function success(fun,afterajax_ok){
                    ...
                    self_validation.on('success.form.fv', function(e) {
                        ...
                        if(ajax_ok){
                            setTimeout(function(){
                                var $modal_btn_publish=$self.parents('.modal').find('.modal-footer .btn-publish');
                                if($modal_btn_publish.length && $self.find('[name="displaytype"]').val()!='-1'){
                                    M.load('alertify',()=>{
                                        alertify.confirm('确认发布该内容？', function (ev) {
                                            formDataAjax(e,fun);
                                        });
                                    });
                                }else{
                ...
                function formDataAjax(e,fun,data){
                        ...
                        if($table.length){
                            ...
                            $self.find('[name]').each(function(index, el) {
                                if($(this).parents('tbody').length?(not_validate_checked?1:($(this).parents('tr').find('.checkall-item[name="id"]').length?$(this).parents('tr').find('.checkall-item[name="id"]:checked').length:1)):1){
                                    if($(this).attr('type')=='radio'?$(this).prop('checked'):1){
                    ...
                    var this_ajax=function(){
                            M.ajax({
                                ...
                                success: function(result) {
                                    ...
                                    if($modal_ok_btn.length){
                                        $modal_ok_btn.hasClass('btm-publish')&&$modal_ok_btn.removeAttr('data-ok');
                                        $modal_ok_btn.clickLoading('stop');
                                    }
                    ...
                    if(handle_name.length){
                        M.ajax({
                            url: M.url.system+'entrance.php?m=include&c=sysinfo&a=doGetinfo&lang='+M.lang,
                            success: function(result) {
                ...
                M.load('formValidation',function(){
                    if(name){
                        ...
                        $.each(name, function(index, val) {
                            $form.data('formValidation').removeField(val);
                        ...
                        $name.each(function(index, el) {
                            ...
                            if($.inArray(name, name_array)<=0){
                                ...
                                if(typeof $(this).attr('required') !='undefined'){
                                    $form.data('formValidation')._cacheFields[name] && $form.data('formValidation').removeField(name);
    ...
    $(document).on('submit', 'form', function(event) {
        var action=$(this).attr('action'),
            not_validate_checked=action=='#'||action=='javascript:;',
            disabled_submit=action.indexOf('http')<0&&action.substr(0,3)!='../',
            $self=$(this),
            $table=$(table_selector,this).filter(function(index, el){return !$(this).parents('.metadmin-fmbx').length;});
        ...
        if(!not_validate_checked && $table.length && ($table.find('tbody .checkall-item[name="id"]').length?!$table.find('tbody .checkall-item[name="id"]:checked').length:0) && $('[name="allid"]',this).val()==''){
            event.preventDefault();
            M.load('alertify',function(){alertify.alert(METLANG.jslang3||'请选择至少一项','','','tips');});
