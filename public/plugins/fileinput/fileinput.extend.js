...
(function() {
    ...
    M.load('modal',function(){
        ...
        M.component.modal_options['.img-library-modal'] = {
            ...
            modalHeaderappend: `<div class="float-right mr-3">
                <button type="button" class="btn btn-warning btn-sm float-left mr-3 invisible btn-img-folder-back"><i class="fa-reply mr-2"></i>${METLANG.back_folder_list}</button>
                <div class="img-library-sort clearfix float-right">
                    ...
                    <a href="javascript:;" title="按更新时间顺叙" data-type="time_asc" class="sort-time text-center text-grey float-left px-2"><i class="fa-clock-o"></i><br><i class="fa-sort-up"></i></a>
                    <a href="javascript:;" title="按名称顺序" data-type="name_asc" class="text-grey float-left px-2"><i class="fa-sort-alpha-asc"></i></a>
                    <a href="javascript:;" title="按名称倒序" data-type="name_desc" class="text-grey float-left px-2"><i class="fa-sort-alpha-desc"></i></a>
                </div>
            </div>`,
            modalSize: 'xl',
            modalFullheight: 1,
	...
	$.fn.extend({
		...
		metFileInput: function() {
			...
			var messageFun = function(obj, type, msg) {
				...
				errorFun = function(obj, data) {
					var $file_input = obj.parents('.file-input'),
						...
						$file_preview_view=$file_input.find(`.file-preview-thumbnails .file-preview-frame .file-preview-view[data-order="${order}"]:not([data-size])`);
					if($file_preview_view.length){
						setTimeout(()=>{
							ignore_hide_message=1;
							$file_preview_view.parents('.file-preview-frame').find('.kv-file-remove').click();
						},0);
					}
					// 显示报错文字
			...
			$(this).each(function(index, el) {
				...
				var fn_key=$(this).attr('data-fn_key');
				$(this).removeAttr('hidden').fileinput({ // fileinput插件
					...
					compliance:0/* parseInt(M.met_agents_metmsg||0) */,
					ajaxSettings: M.fileinput_fn[fn_key]&&M.fileinput_fn[fn_key].ajaxSettings?(typeof M.fileinput_fn[fn_key].ajaxSettings=='function'?M.fileinput_fn[fn_key].ajaxSettings():M.fileinput_fn[fn_key].ajaxSettings):{},
					uploadFileAttr:$(this).attr('data-upload_file_attr')||'',
					uploadExtraData:function(){
						return M.fileinput_fn[fn_key]&&M.fileinput_fn[fn_key].uploadExtraData?(typeof M.fileinput_fn[fn_key].uploadExtraData=='function'?M.fileinput_fn[fn_key].uploadExtraData():M.fileinput_fn[fn_key].uploadExtraData):{}
					},
					...
					var $self = $(this),
						$is_compress = $(this).parents('.file-input').find('.file-preview-frame:not(.file-preview-initial) img').filter(function(index) {
							var result = this.naturalWidth > 2600 || this.naturalHeight > 2600;
							return result;
						}),
					...
					if (compress_length) {
						...
						$is_compress.each(function(index, el) {
							...
							var canvas = document.createElement('canvas'),
								...
								size = imgScale(2600, 2600, this.naturalWidth, this.naturalHeight),
						...
						if(M.fileinput_fn[fn_key] && M.fileinput_fn[fn_key].upload){
							M.fileinput_fn[fn_key].upload(files);
							return;
						}
				...
				// 上传按钮组样式优化
				setTimeout(()=>{
					$(this).parents('.file-input').find('.input-group .btn-group-file .btn:visible').addClass('element-visible');
					value && value.indexOf(delimiter)>0 && $(this).parents('.file-input').find('.file-preview .close').show();
				}, 100);
			});
		},
		// 上传图片组件改变值
	...
	function fileThumbnail(imgurl) {
		var imgurl_handle=imgurl.toLowerCase(),
			suffix_list={
				image:['png','jpeg','jpg','bmp','gif','ico','svg','webp'],
				video:['mp4','webm','ogg']
			},
			file_html = (()=>{
				var html='',
            ...
            <div class="file-thumbnail-footer">
                ...
                <div class="file-actions d-flex">
                    <a href="${imgurl}" class="btn btn-xs btn-light file-preview-view" title="${METLANG.clickview}" target="_blank"><i class="fa-eye text-grey"></i></a>
                    <div class="file-footer-buttons d-flex">
                        <button type="button" class="kv-file-remove btn btn-xs btn-light" title="${$.fn.fileinputLocales[M.fileinput_lang].removeTitle}"><i class="fa-trash-o text-grey"></i></button>
                    </div>
	...
	$(document).on('click', '.file-input .file-preview-thumbnails .file-preview-frame .kv-file-remove,.fileinput-remove', function(event) {
		...
		var $file_input = $(this).parents('.file-input'),
			...
			required = parseInt($file.attr('data-filerequired')||0),
		...
		if(!ignore_hide_message){
			if(!(required && !$input_name.val())) $(this).parents('.form-group').removeClass('has-success has-danger');
			$(this).parents('.form-group').find('span.form-control-label').remove();
		}else{
			setTimeout(()=>{
				ignore_hide_message=0;
			},0);
		}
	});
	// 文件地址更新后，图片尺寸更新
	$(document).on('change', 'form .fileinput-input-text[name]', function(event) {
		...
		setTimeout(()=>{
			...
			var val=$(this).val()||'';
			$(this).parents('.file-input').find('.file-preview .close')[val.indexOf(delimiter)>0?`show`:'hide']();
			// 自定义回调
			var callback = $file.data('callback');
			callback && eval(callback+'($file,val)');
		}, 0);
	});
	...
	$(document).on('click', '.img-library-modal button[data-ok]', function(event) {
		...
		M.load('alertify', function() {
			if (img_url) {
				...
				$file.parents('.form-group').removeClass('has-success has-danger').find('span.form-control-label').remove();
	...
	$(document).on('click clicks', '.img-library-sort a', function(event) {
		$(this).addClass('active').removeClass('text-grey').siblings('a').removeClass('active').addClass('text-grey');
		if(event.type=='click'){
