/**
 * User: Jinqn
 * @Date：2023-05-26 17:29:33
 * 修改：米拓企业建站系统 Copyright (C) 长沙米拓信息技术有限公司 (https://www.metinfo.cn). All rights reserved.
 ...
 (function () {
    ...
    RemoteImage.prototype = {
        ...
        initContainer: function () {
            ...
            this.img && this.setImage(this.img);
        },
        initEvents: function () {
            var _this = this,
                locker = $G('lock');
            ...
            /* 改变url */
            domUtils.on($G("url"), 'keyup', function(){updatePreview('url')});
        ...
        updateLocker: function(){
            ...
            if(width && height && width>0 && height>0) {
                ...
                locker.title =lockicon.title= lang.lockicon;
                // $('.lockicon-tip').html('');
            } else {
                locker.checked = false;
                locker.disabled = 'disabled';
                locker.title =lockicon.title= lang.remoteLockError;
                // $('.lockicon-tip').html(lang.remoteLockError);
            }
        },
        setImage: function(){
            /* 不是正常的图片 */
            if (!this.img.tagName || this.img.tagName.toLowerCase() != 'img' && !this.img.getAttribute("src") || !this.img.src) return;

            var wordImgFlag = this.img.getAttribute("word_img"),
                src = wordImgFlag ? wordImgFlag.replace("&amp;", "&") : (this.img.getAttribute('_src') || this.img.getAttribute("src", 2).replace("&amp;", "&")),
            ...
            if(src) {
                /* 设置表单内容 */
                $G("width").value = this.img.getAttribute("width") || $(this.img).width();
                $G("height").value = this.img.getAttribute("height") || $(this.img).height();
                $G("border").value = this.img.getAttribute("border") || '0';
                $G("vhSpace").value = this.img.getAttribute("vspace") || '0';
                $G("title").value = this.img.title || this.img.alt || '';
                setAlign(align);
                this.setPreview();
                this.updateLocker();
            }
        ...
        setPreview: function(type){
            ...
            if(url) {
                var is_new_url=url!=$(preview).find('img').attr('src');
                if(is_new_url) preview.innerHTML = '<img src="' + url + '"/>';
                $(preview).find('img').css({border:border + 'px solid #000',title:title});
                var handle=()=>{
                        var ow = $G('width').value,
                            oh = $G('height').value,
                            width=!ow? preview.offsetWidth:Math.min(ow, preview.offsetWidth),
                            height;
                        height = (!ow || !oh) ? '':width*oh/ow;
                        $(preview).find('img').attr({width:width,height:height});
                    }
                if(type=='url' && is_new_url){
                    var new_img_size=(img)=>{
        ...
        getInsertList: function () {
            ...
            if(data.url) {
                var $preview_img=$($G('preview')).find('img'),
                    new_size=($preview_img[0].naturalWidth!=parseInt(data.width||0)&&data.width)||($preview_img[0].naturalHeight!=parseInt(data.height||0)&&data.height);
                return [{
                    src: data.url,
                    _src: data.url,
                    width: new_size?data.width:'',
                    height: new_size?data.height:'',
                    border: data.border || '',
                    ...
                    title: data.title || '',
                    alt: data.title || '',
                    style:new_size? "width:" + data.width + "px;height:" + data.height + "px;":''
                }];
            } else {
                return [];
            }
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('fileQueued', function (file) {
                ...
                addFile(file);
                // 上传图片列表拖拽排序
                if(!$('.dragsort-js').length){
                    $('body').append(`<script type="text/javascript" class='dragsort-js' src="../../../dragsort/jquery.dragsort.js"></script>`);
                    var interval=setInterval(()=>{
                        if(typeof $.fn.dragsort!='undefined'){
                            clearInterval(interval);
                            $queue.dragsort({
                                dragSelector: $queue.data('drag_selector'),
                                dragBetween: false ,// 设置为“true”，如果你要启用多组列表之间拖动选定的列表。 默认值是false。
                                dragSelectorExclude:typeof $queue.data('drag_selector_exclude')!='undefined'?$queue.data('drag_selector_exclude'):'input,textarea,select,a,i,span,button,.minicolors-panel',
                            });
                        }
                    },200);
                    $statusBar.find('.upload-tips').text(lang.dragImage);
                }
            });
        ...
        init: function () {
            ...
            $(document).on('click clicks', '.alignBar-online a', function(event) {
                ...
                if(event.type=='click'){
                    ...
                    setCookie('ueditor-onlineimage-sort',$(this).data('type'),setCookie?'':{path:'/',expires:365},'365');
                }
            });

            var sort=getCookie('ueditor-onlineimage-sort');
