...
define(function(require, exports, module) {
	...
	/*增加对$.browser.msie的支持*/
	require('third/admin_old/jquery-migrate/jquery-migrate-1.2.1.min');
	function AssemblyLoad(dom){
		/*上传组件*/
		if(dom.find('.ftype_upload .fbox input').length){
			require.async('epl/upload/set',function(a){
				a.func(dom);
			});
		}
		/*编辑器*/
		...
		if(dom.find('.ftype_range .fbox input').length){
			require.async('epl/range/set',function(a){
				a.func(dom);
			});
		}
		/*日期选择器*/
		if(dom.find('.ftype_day input').length){
			require.async('epl/time/set',function(a){
				a.func(dom);
			});
		}
		/*联动菜单*/
		if(dom.find('.ftype_select-linkage .fbox').length){
			require.async('epl/select-linkage/set',function(a){
				a.func(dom);
			});
		}
		/*标签增加器*/
		if(dom.find('.ftype_tags').length){
			require.async('epl/tags/set',function(a){
				a.func(dom);
			});
		}
		/*表格控件*/
		if(dom.find('.ui-table:not([data-noajax])').length){
			require.async('epl/table/set',function(a){
				a.func(dom);
			});
		}
		// 图标选择
