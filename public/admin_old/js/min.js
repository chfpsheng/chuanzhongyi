...
define(function(require, exports, module) {
	...
	/*cookie*/
	require('third/admin_old/cookie/cookie');
	//adminlang = $.cookie('langset');当前后台语言
	//bootstrap
	require('third/admin_old/bootstrap/bootstrap.min');
	// 系统参数
	...
	window.M={
		...
		url:{
			...
			public_third:siteurl + 'public/third-party/',
	...
	/*---------页面组件加载---------*/
	/*表单验证*/
	if($('form.ui-from').length>0)require.async('epl/form/set');
	...
	$(document).on('click','.met-mask',function(){
		require.async('web_plugins/alertify/alertify.js',function(a){
			alertify.theme('bootstrap').okBtn(METLANG.confirm).cancelBtn(METLANG.jslang2).confirm('<span class="text-danger met-check-login">'+(METLANG.jsx32||'登录超时，请重新登录！')+'</span>', function (ev) {
	...
	setInterval(()=>{
		if($.cookie('met_auths')){
			$('.met-mask').length && $('.met-mask').remove();
		}else{
			!$('.met-mask').length && $('html').append(`<div class='met-mask' style="position:fixed;width:100%;height:100%;left:0;top:0;z-index:999;"></div>`);
		}
	},2000);
});
// 弹出提示信息
