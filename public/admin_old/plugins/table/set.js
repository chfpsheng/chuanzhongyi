...
define(function(require, exports, module) {
	...
	function tablexp(obj){
		...
		if(obj.attr('data-table-datatype') == 'jsonp'){
			require('third/admin_old/table/js/jquery.dataTables.jsonp');
		}else{
			require('third/admin_old/table/js/jquery.dataTables.min');
		}
	 	// setTimeout(function(){
	 			...
	 			window.datatableOption=function(obj){
		            ...
		            var language=langset!='cn'&&langset!='en'?'cn':langset,
		            	option={
							...
							language: { // 语言配置文件
								url: siteurl + 'public/third-party/admin_old/table/lang/'+language+'.php'
							},
							ajax: {
							...
							drawCallback: function(settings){// 表格重绘后回调函数
					            ...
					            if($('[data-original]',this).length){
					            	var $self=$(this);
					            	require.async('third/jquery-lazyload/jquery.lazyload.min.js',function(a){
