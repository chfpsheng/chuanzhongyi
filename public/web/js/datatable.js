...
$.fn.metDataTable=function(){
    ...
    if($datatable.length){
        ...
        // }
        var datatable_langurl= M.weburl+'public/third-party/web/datatables/language/';
        // datatable多语言选择
            ...
            var new_option=datatable_option[datatable_order]||'',
                option={
                    ...
                    initComplete: function(settings, json) {// 表格初始化回调函数
                        ...
                        if(pagenum>2){
                            // 跳转到某页
                            var gotopage_html='<div class="gotopage inline-block m-t-15 m-l-10"><span>'+(M.lang=='cn'?'跳转至':'Go to')+'</span> <input type="number" name="gotopage" min="1" max="'+pagenum+'" class="form-control form-control-sm w-50 text-xs-center"/> '+(M.lang=='cn'?'页':'page')+' <input type="button" class="btn btn-default btn-sm gotopage-btn" value="'+(M.lang=='cn'?'跳转':'to')+'"/></div>';
                            ...
                            $gotopage.find('.gotopage-btn').click(function(event) {
                                ...
                                if(!isNaN(gotopage)){
                                    if(gotopage>=1&&gotopage<=pagenum){
                                        ...
                                        alert((M.lang=='cn'?'页码有效范围为：':'The valid range of page number is:')+'1~'+pagenum);
                                    }
                                }
                            });
                        }
