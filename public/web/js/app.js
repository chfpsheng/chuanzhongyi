...
$.extend(M.plugin,{
    ...
    checkall:M.url.public_plugins+'checkall/checkall.js',
});
...
$.fn.extend({
                    ...
                    $.ajax({
                        ...
                        success:function(result){
                            if(parseInt(result.status)){
                                ...
                                $loader.hide();
                                $list.html(html);
                                M.plugins_events.lazyload($list,{container:'.img-library-body'});
    ...
    metCommon:function(){
        ...
        // 全选、全不选
        $('[data-plugin="checkAll"]',dom).loadPlugin('checkall',M.plugin.checkall,function(){
            return $.fn.checkAll;
        },function(obj) {
            obj.checkAll('.checkall-all','.checkall-item:not([disabled])','.checkall-contrary');
        });
    }
});
// 通用功能开启
